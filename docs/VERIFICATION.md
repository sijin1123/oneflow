...
# UI-278 Project Intake Composition 검증 (2026-07-28)

- **UI 변경**: Project Intake를 큰 상태 카드와 분리된 요청 목록에서 중앙 콘텐츠 프레임에 직접 결합된 compact queue surface로 재구성했다. 실제 refresh와 Add request를 프레임 상단에 유지하고, 열린 요청·대기·보류·수락·종료 집계, inline composer, 상태별 dense rows, 판정 입력과 이력을 한 정보 계층으로 통합했다. desktop과 `390x844` mobile 모두 문서 가로 넘침 없이 동일한 기능 순서를 유지한다.
- **기능/API 반영**: 기존 member submit, 서버 범위 queue, owner 및 custom-role `intake.triage` 판정, archive/viewer write boundary, accepted work-item deep link와 lazy decision history를 그대로 연결했다. 제출·판정 실패는 정확한 제목·메모·판정 payload를 유지해 명시적으로 재시도한다. 403/409는 즉시 프로젝트별 fail-close 상태로 전환하고 queue/me/member/project/permission 재조회가 모두 성공한 경우에만 쓰기를 다시 연다. 프로젝트를 전환하면 입력·오류·highlight를 격리하고 이전 프로젝트의 늦은 재조회 성공이 현재 프로젝트의 쓰기 잠금을 해제하지 못한다. 초기 loading은 최종 geometry를 닮은 skeleton으로 표시하고, queue/access 초기 오류와 last-success 재조회 오류는 화면을 비우지 않고 실제 query로 복구한다. 신규 API, DB/schema, migration, permission, 환경변수, dependency 또는 Settings UI 변경은 없다.
- **이연 항목**: 없음. 제출, Enter submit, refresh, 수락·거절·중복·보류, 실패 재시도, 충돌 새로고침, 작업 deep link, 판정 이력, owner/member/viewer/custom-role/archive 상태가 모두 기존 실제 query/mutation 또는 navigation에 연결돼 있으며 mock/dead control은 없다.
- **검증**: typecheck PASS, lint PASS(기존 Fast Refresh warning 4건), production build PASS(기존 large chunk warning), unit **115 PASS**, component **9 PASS**, Intake focused E2E **16 PASS**다. 프로젝트별 권한 latch까지 반영한 4-worker 전체 E2E는 **455 PASS + retry-resolved flaky 2 + opt-in visual QA manifest 1 skip**로 종료 코드 0을 기록했다. flaky는 이번 변경과 무관한 Workspace Home 빠른 링크 메뉴와 Project cover modal close timing이며, 두 시나리오를 1-worker로 각각 3회 반복해 **6/6 PASS**를 확인했다. 이후 독립 검토에서 발견한 늦은 이전-project submit continuation은 project-keyed surface remount와 Strict Mode-safe mounted guard로 닫고 success/503/409 전환 회귀를 추가해 focused **16/16 PASS**, 새 전환 시나리오 반복 **3/3 PASS**를 확인했다. 제출·판정 exact retry, delegated triage, ordinary member, viewer/archive read-only, supporting-query loading, retained-data recovery, dynamic archive, delegated-permission revocation, concurrent-decision 409 full refresh, deep-link focus/re-entry, 프로젝트 전환과 늦은 refresh/submit 응답 격리, stable loading/error geometry, desktop/mobile zero horizontal overflow와 lazy history를 검증했다. 독립 검토에서 403/409 queue-only refresh, supporting-query loading flicker, deep-link focus 누락, refetch 실패 시 stale write 권한 유지와 cross-project late-response race를 발견해 project-scoped fail-close, 5-query recovery gate와 stable skeleton/focus 계약으로 닫았다. Clean-room frontend **182**/backend **45**, OpenAPI type parity와 diff check도 PASS했다. 전체 E2E가 생성한 기존 screenshot rewrite 300여 건과 로그인 임시 캡처는 테스트 산출물로 원복·삭제하고 UI-278 증적만 보존했다.
- **보안 감사**: 신규 dependency/lockfile 변경은 없다. 2026-07-28 기준 `npm audit --omit=dev --audit-level=high`는 React Router RSC mode의 신규 공개 권고 `GHSA-qwww-vcr4-c8h2`로 high 2건을 보고했다. OneFlow는 RSC mode를 사용하지 않으며 CI security-audit는 비차단 추적, 버전 교체는 Dependabot의 별도 dependency PR 범위다.
...
# UI-280 Project Meetings Composition 검증 (2026-07-28)

- **UI 변경**: Project Meetings 목록을 큰 소개 header와 분리된 summary cards에서 중앙 콘텐츠 프레임에 직접 결합된 compact directory surface로 재구성했다. 프레임 상단에 실제 refresh와 permission-aware 새 회의를 유지하고, 전체·일정 있음·일정 미정·반복 집계, URL-backed 제목 검색, 템플릿 선택, dense meeting rows를 한 정보 계층으로 통합했다. desktop과 `390x844` mobile 모두 문서 가로 넘침 없이 동일한 기능 순서를 유지한다.
- **기능/API 반영**: 기존 meeting/template/project query, 프로젝트 멤버십과 archive write boundary, template-backed meeting create, 상세 navigation을 그대로 연결했다. 생성 실패는 정확한 template ID와 이름을 보존해 동일 payload로 명시적 재시도한다. 초기 loading/error에서도 최종 프레임 action과 geometry를 유지하고, 마지막 성공 데이터의 재조회 오류는 목록을 비우지 않은 채 실제 meeting/project/template query로 복구한다. 프로젝트 전환 시 검색 외 local mutation 상태를 프로젝트별로 격리하며 viewer/archive 상태에서는 생성 control과 템플릿 선택을 노출하지 않는다. 신규 API, DB/schema, migration, permission, 환경변수, dependency 또는 Settings UI 변경은 없다.
- **이연 항목**: 회의 상세의 title/date/recurrence 편집, agenda/minutes, action item, follow-up, template save 기능은 이번 목록 PR에서 축소하거나 mock으로 대체하지 않고 기존 실제 구현을 유지했으며 집중 회귀로 검증했다. 해당 상세 화면의 UI-first 정보 구조 재구성은 다음 독립 surface PR인 UI-281 범위로 이연한다.
- **검증**: typecheck PASS, lint PASS(기존 Fast Refresh warning 4건), production build PASS(기존 large chunk warning), unit **115 PASS**, component **9 PASS**, Meetings focused E2E **9 PASS**다. 4-worker 전체 E2E는 UI-280 포함 **453 PASS + opt-in visual QA manifest 1 skip** 후 11건이 장시간 병렬 자원 포화와 animation 관측 경합으로 실패했으며, 실패한 서로 무관한 11건을 1-worker로 다시 실행해 **11/11 PASS**를 확인했다. UI-280의 신규 3건과 기존 회의 6건은 첫 전체 실행에서도 모두 PASS했다. Clean-room frontend **182**/backend **45**, OpenAPI type parity와 diff check도 PASS했다. 전체 E2E가 생성한 기존 screenshot rewrite 349건과 로그인 임시 캡처 3건은 테스트 산출물로 원복·삭제하고 UI-280 증적만 보존했다.
- **보안 감사**: 신규 dependency/lockfile 변경은 없다. `npm audit --omit=dev --audit-level=high`는 React Router RSC mode 권고 `GHSA-qwww-vcr4-c8h2`로 기존 high 2건을 보고했다. OneFlow는 RSC mode를 사용하지 않으며 강제 수정은 `react-router-dom@7.11.0`으로의 breaking downgrade를 요구하므로 별도 dependency PR에서 추적한다.
