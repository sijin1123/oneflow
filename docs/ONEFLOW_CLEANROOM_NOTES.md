...
## UI-189 Work Item Custom Field Properties IA

`docs/plane-poc-reverse-spec/`에서 사용한 입력은 작업 상세의 확장 속성이 compact count/header/label-value hierarchy, 타입 단서, 보존값과 독립 loading/error recovery를 제공한다는 S027의 관찰 가능한 정보 구조와 행동뿐이다. OneFlow 구현은 기존 custom-field definition/value API hooks, React Query, React state와 자체 design token/Lucide icon으로 독립 설계했다.

Plane/OpenProject source, API, database schema, package, asset, CSS, DOM hierarchy, exact dimensions/colors, wording, component implementation, icons 또는 branding은 복사하지 않았다. text/number/URL/boolean/date/dropdown/member 입력, delta PUT, removed option/member 보존, writer/viewer 경계와 mobile flow를 기존 OneFlow 계약에 유지했고 신규 API, DB/schema, migration, permission, environment variable, dependency 또는 Settings storage 변경은 없다. - attested-independent-derivation

## UI-190 Work Item Description IA
...
## UI-192 OneFlow AI Ask/Build Workspace
...
## UI-203 Login Pixel Exhaustive Closure
...
## UI-207A Login Origin Pixel Reinspection

UI-207A의 유일한 시각 입력은 사용자가 직접 제공하고 OneFlow 제품 자산으로 승인한 `apps/web/src/assets/generated/oneflow-login-origin-reference.png`이며 SHA-256은 `62fafe9e44df9d189e8fe2f38fc25147d11b8459569be13ee0424ba06c0c4c76`이다. 런타임 story와 양쪽 OneFlow logo는 이 원본의 결정적 crop이고, 재실사에서 story brand·headline·Kanban·calendar·activity와 auth logo가 각각 pixel MAE `0.0000`임을 확인했다. 배경과 로고를 새로 생성·재해석하지 않았으며 기능형 인증 DOM의 구분선 좌표·길이·색상만 원본과 대조해 보정했다.
...
## UI-209 Inbox Notification Identity Avatars
...
`docs/plane-poc-reverse-spec/04-interaction-catalog.md`와 `11-focused-sidebar-views-observations.md`에서 사용한 입력은 같은 작업 범위를 Table, Board, Calendar, Timeline으로 전환하고, query·배율·기준일을 복원하며, 일정 막대·의존관계·마일스톤과 실제 생성·상세 action을 연결한다는 관찰 가능한 정보 구조와 행동뿐이다. Plane/OpenProject source, API, database schema, package, asset, CSS, DOM hierarchy, exact dimensions/colors, wording, component implementation, icons 또는 branding은 열람·복사하지 않았다.

OneFlow 구현은 기존 자체 Work Package query/create/draft, project membership, relation/milestone, versioned schedule PATCH, drawer/full-page/move/duplicate contract, MIT 고정 DHTMLX Gantt, React Router와 local token/Lucide primitive 위에서 독립 설계했다. `scale`과 `focus`는 검증된 canonical URL 상태이고 검색·필터·권한·card action·actual create와 stable loading/error/filtered-empty가 같은 timeline canvas 안에서 동작한다. 신규 API, DB/schema, migration, permission, environment variable, dependency 또는 Settings storage 변경은 없고 mock/dead control이나 client-only schedule item도 없다. - attested-independent-derivation
...
`docs/plane-poc-reverse-spec/03-feature-matrix.md`, `04-interaction-catalog.md`와 기존 OneFlow 파일 계약에서 사용한 입력은 프로젝트 자료를 compact directory로 스캔하고, 검색·범위·업로드·외부 링크·연결 대상·본문 검색 준비·실패 복구를 실제 action과 연결한다는 관찰 가능한 정보 구조와 행동뿐이다. 별도의 Plane 파일 API나 구현은 참조하지 않고 OneFlow가 이미 보유한 attachment directory, binary storage, body index, work-item/document anchor, pagination과 authorization 계약을 우선했다.

Plane/OpenProject source, API, database schema, package, asset, CSS, DOM hierarchy, exact dimensions/colors, wording, component implementation, icons 또는 branding은 열람·복사하지 않았다. OneFlow 구현은 기존 attachment/document/work-item/project query와 mutation, canonical URL search/scope, React Query retained data, local design token과 Lucide primitive 위에서 독립 설계했다. 신규 API, DB/schema, migration, permission, environment variable, dependency 또는 Settings storage 변경은 없고 mock/dead control, 가짜 파일 또는 client-only index transition은 없다. - attested-independent-derivation

## UI-280 Project Meetings Composition

`docs/plane-poc-reverse-spec/03-feature-matrix.md`, `04-interaction-catalog.md`와 기존 OneFlow 회의 계약에서 사용한 입력은 협업 목록이 compact 상태 요약, 검색·생성 제어, 목록과 상세 이동을 일관된 정보 구조로 제공한다는 관찰 가능한 행동 범주뿐이다. 별도의 Plane 회의 API나 구현은 참조하지 않고 OneFlow가 이미 보유한 meeting/template, 프로젝트 멤버십 권한, archive write boundary, action item과 follow-up 계약을 우선했다.

Plane/OpenProject source, API, database schema, package, asset, CSS, DOM hierarchy, exact dimensions/colors, wording, component implementation, icons 또는 branding은 열람·복사하지 않았다. OneFlow 구현은 기존 meeting/template/project query와 mutation, canonical URL 제목 검색, React Query retained data, local design token과 Lucide primitive 위에서 독립 설계했다. 템플릿 생성 실패는 정확한 선택 의도를 보존해 재시도하고 loading/error/retry/empty/read-only와 desktop/mobile geometry를 같은 surface에서 유지한다. 신규 API, DB/schema, migration, permission, environment variable, dependency 또는 Settings storage 변경은 없고 mock/dead control이나 client-only meeting은 없다. - attested-independent-derivation

## UI-281 Project Meeting Detail Composition

`docs/plane-poc-reverse-spec/03-feature-matrix.md`, `04-interaction-catalog.md`와 기존 OneFlow 회의 상세 계약에서 사용한 입력은 협업 상세가 편집 가능한 핵심 내용, 일정·반복 속성, 후속 흐름과 실행 항목을 한 compact surface에서 연결하고 loading/error/retry를 실제 상태로 표현한다는 관찰 가능한 정보 구조와 행동뿐이다. 별도의 Plane 회의 상세 API나 구현은 참조하지 않고 OneFlow가 이미 보유한 versioned meeting PATCH, action item, follow-up, template, project membership와 archive write boundary를 우선했다.

Plane/OpenProject source, API, database schema, package, asset, CSS, DOM hierarchy, exact dimensions/colors, wording, component implementation, icons 또는 branding은 열람·복사하지 않았다. OneFlow 구현은 기존 meeting/project query와 mutation, React Query retained data, local design token, Lucide primitive와 자체 rich-text editor 위에서 독립 설계했다. 제목·일정·반복·안건·회의록 초안과 409 충돌을 보존하고, 저장·후속 회의·템플릿·삭제·액션 아이템 실패는 안전한 범위에서 정확한 의도를 재시도한다. 신규 API, DB/schema, migration, permission, environment variable, dependency 또는 Settings storage 변경은 없고 mock/dead control, 장식용 activity tab 또는 client-only meeting state는 없다. - attested-independent-derivation
