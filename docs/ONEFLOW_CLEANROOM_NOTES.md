...
| Reverse Spec 기반 workspace all-work grid | `docs/plane-poc-reverse-spec/`의 D004/S009-S015 all work items grid와 RSP-003 backlog | `plane/apps/**`, `plane/packages/**` | 워크스페이스 단위로 접근 가능한 모든 작업을 프로젝트/상태/우선순위/담당자/일정 컬럼의 가로 스크롤 표로 탐색 | 기존 OneFlow search membership boundary를 확장한 `/search/work-packages` 계약과 신규 `/work-items` 페이지를 자체 구현. Plane table DOM/CSS/컬럼 메뉴/카피/에셋 없이 OneFlow table/chip primitives와 API tests로 독립 구현 |
