...
function WikiPolicySkeleton() {
  return (
    <div role="status" aria-label="Wiki 정책 불러오는 중" className="space-y-5 py-5">
      <span className="sr-only">Wiki 정책을 불러오는 중입니다.</span>
...
export function WikiSettingsPage() {
  ...
  const changePolicy = (enabled: boolean) => {
    ...
    update.mutate(
      ...
      {
        onSuccess: () => {
          setSuccessMessage(enabled ? 'Wiki를 활성화했습니다.' : 'Wiki를 비활성화했습니다.')
        },
        onError: () => {
  ...
  return (
    <div className="flex h-full min-w-0 flex-col bg-of-surface">
      ...
      <div
        data-testid="wiki-settings-scroll"
        ...
        <div className="mx-auto w-full max-w-5xl px-4 py-5 sm:px-6 sm:py-6">
          <header className="grid gap-4 border-b border-of-border pb-5 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-end">
            <div className="min-w-0">
              ...
              <h1 className="mt-1 text-xl font-semibold">Wiki</h1>
              <p className="mt-1 max-w-2xl text-xs leading-5 text-of-muted">
                프로젝트 문서, 작업 연결과 통합 검색의 워크스페이스 정책을 관리합니다.
              </p>
            </div>
            {data ? (
          ...
          {policy.isPending ? <WikiPolicySkeleton /> : null}
          ...
          {data ? (
            <div className="space-y-5 py-5">
              {policyStale ? (
                ...
                >
                  <p className="min-w-0 flex-1">
                    최신 Wiki 정책을 불러오지 못해 마지막으로 확인한 상태를 유지합니다. 복구 전까지 정책 변경은 사용할 수 없습니다.
                  </p>
              ...
              >
                ...
                />
                <PolicyFact label="저장 데이터" value="보존됨" />
              </dl>

              <section aria-labelledby="wiki-policy-title">
                  ...
                  <div className="flex min-w-0 items-start gap-3">
                    ...
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        <h2 id="wiki-policy-title" className="text-sm font-semibold">
                          프로젝트 Wiki
                        </h2>
                      ...
                      <p className="mt-1 max-w-2xl text-xs leading-5 text-of-muted">
                        문서 작성, 편집, 코멘트, 작업 연결과 통합 검색을 함께 제어합니다.
                      </p>
                    </div>
                  </div>
                  <button
                  ...
                  {update.isError ? (
                    <div className="flex min-w-0 flex-wrap items-center justify-between gap-2">
                      ...
                      {failedTarget !== null ? (
                        ...
                        >
                          {failedTarget ? 'Wiki 켜기 다시 시도' : 'Wiki 끄기 다시 시도'}
                        </Button>
                      ) : null}
                    </div>
                  ) : null}
              ...
              <div className="grid gap-5 lg:grid-cols-[minmax(0,1.35fr)_minmax(16rem,0.65fr)]">
                <section aria-labelledby="wiki-policy-effects-title">
                  <div className="border-b border-of-border-subtle pb-3">
                    <h2 id="wiki-policy-effects-title" className="text-sm font-semibold">
                      정책 영향 범위
                    </h2>
                    <p className="mt-1 text-xs leading-5 text-of-muted">
                      하나의 정책이 실제 탐색과 서버 접근에 동일하게 적용됩니다.
                    </p>
                  </div>
                  <ul className="divide-y divide-of-border-subtle border-b border-of-border-subtle">
                    <li className="flex min-w-0 items-start gap-3 py-3">
                      ...
                      <div className="min-w-0">
                        <p className="text-xs font-medium">탐색 메뉴</p>
                        <p className="mt-0.5 text-xs leading-5 text-of-muted">
                          {data.enabled
                            ? 'Wiki와 프로젝트 Documents 진입점을 표시합니다.'
                            : 'Wiki와 프로젝트 Documents 진입점을 숨깁니다.'}
                        </p>
                      </div>
                    </li>
                    <li className="flex min-w-0 items-start gap-3 py-3">
                      ...
                      <div className="min-w-0">
                        <p className="text-xs font-medium">검색과 문서 API</p>
                        <p className="mt-0.5 text-xs leading-5 text-of-muted">
                          {data.enabled
                            ? '통합 검색과 문서 작성·편집·연결 API를 사용할 수 있습니다.'
                            : '통합 검색의 문서 결과를 숨기고 문서 API 요청을 차단합니다.'}
                        </p>
                      </div>
                    </li>
                    <li className="flex min-w-0 items-start gap-3 py-3">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="mt-0.5 text-xs leading-5 text-of-muted">
                          정책을 꺼도 기존 문서와 연결은 삭제되지 않으며 다시 켜면 복구됩니다.
                        </p>
                      </div>
                    </li>
                  </ul>
                ...
                <section aria-labelledby="wiki-policy-audit-title">
                  <div className="border-b border-of-border-subtle pb-3">
                    <h2
                      id="wiki-policy-audit-title"
                  ...
                  <dl className="divide-y divide-of-border-subtle border-b border-of-border-subtle">
                    ...
                    <div className="flex items-start gap-2 py-3 text-xs leading-5 text-of-muted">
                      ...
                      <span>
                        ...
                        {data.enabled ? '문서 기능 사용 가능' : '문서 기능 안전 차단'}
                      </span>
                    </div>
                  </dl>
                </section>
