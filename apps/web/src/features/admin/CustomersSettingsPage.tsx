...
function CustomersPolicySkeleton() {
  return (
    <div role="status" aria-label="Customers 정책 불러오는 중" className="space-y-5 py-5">
      <span className="sr-only">Customers 정책을 불러오는 중입니다.</span>
...
export function CustomersSettingsPage() {
      ...
      { enabled, revision: data.revision },
      {
        onSuccess: () => {
          setSuccessMessage(
            enabled
              ? 'Customers를 활성화했습니다.'
              : 'Customers를 비활성화했습니다. 고객 기록과 작업 연결은 유지됩니다.',
          )
        },
  ...
  return (
    <div className="flex h-full min-w-0 flex-col bg-of-surface">
      <FrameContextActions>
        <Link to="/customers" className={actionClassName}>
      ...
      <div
        data-testid="customers-settings-scroll"
        ...
        <div className="mx-auto w-full max-w-5xl px-4 py-5 sm:px-6 sm:py-6">
          <header className="grid gap-4 border-b border-of-border pb-5 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-end">
            <div className="min-w-0">
              ...
              <h1 className="mt-1 text-xl font-semibold">Customers</h1>
              <p className="mt-1 max-w-2xl text-xs leading-5 text-of-muted">
                고객 기록, 내부 분류와 작업 진행 연결의 워크스페이스 정책을 관리합니다.
              </p>
            </div>
            {data ? (
          ...
          {!data && policy.isPending ? <CustomersPolicySkeleton /> : null}
          ...
          {!data && policy.isError ? (
            <div className="py-5">
              {policy.error instanceof ApiError && policy.error.status === 403 ? (
                <EmptyState
                  title="접근 권한이 없습니다"
                  hint="워크스페이스 Customers 정책은 관리자만 변경할 수 있습니다."
                />
              ) : (
          ...
          {data ? (
            <div className="space-y-5 py-5">
              ...
              >
                ...
                />
                <PolicyFact label="고객 데이터" value="보존됨" />
              </dl>

              <section aria-labelledby="customers-policy-title">
                  ...
                  <div className="flex min-w-0 items-start gap-3">
                    ...
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        <h2 id="customers-policy-title" className="text-sm font-semibold">
                          고객 관리
                        </h2>
                      ...
                      <p className="mt-1 max-w-2xl text-xs leading-5 text-of-muted">
                        고객 디렉터리, 내부 태그, 고객별 진행 집계와 작업 연결을 함께
                        제어합니다.
                      </p>
                    </div>
                  </div>
              ...
              <div className="grid gap-5 lg:grid-cols-[minmax(0,1.35fr)_minmax(16rem,0.65fr)]">
                <section aria-labelledby="customers-effects-title">
                  <div className="border-b border-of-border-subtle pb-3">
                    <h2 id="customers-effects-title" className="text-sm font-semibold">
                      정책 영향 범위
                    </h2>
                    <p className="mt-1 text-xs leading-5 text-of-muted">
                      정책 상태가 고객 탐색, 작업 연결과 서버 접근에 동일하게 적용됩니다.
                    </p>
                  </div>
                  <ul className="divide-y divide-of-border-subtle border-b border-of-border-subtle">
                    <li className="flex min-w-0 items-start gap-3 py-3">
                      <UsersRound size={16} className="mt-0.5 shrink-0 text-of-muted" />
                      <div className="min-w-0">
                        <p className="text-xs font-medium">고객 디렉터리</p>
                        <p className="mt-0.5 text-xs leading-5 text-of-muted">
                          {data.enabled
                            ? '고객 생성, 수정, 보관과 고객별 진행 현황을 사용할 수 있습니다.'
                            : '워크스페이스 고객 디렉터리 진입을 숨기고 요청을 차단합니다.'}
                        </p>
                      </div>
                    </li>
                    <li className="flex min-w-0 items-start gap-3 py-3">
                      <Link2 size={16} className="mt-0.5 shrink-0 text-of-muted" />
                      <div className="min-w-0">
                        <p className="text-xs font-medium">작업 연결과 필터</p>
                        <p className="mt-0.5 text-xs leading-5 text-of-muted">
                          {data.enabled
                            ? '작업 상세의 고객 속성, 목록 필터와 저장 뷰 조건을 표시합니다.'
                            : '작업의 고객 선택과 고객 필터를 화면에서 제외합니다.'}
                        </p>
                      </div>
                    </li>
                    <li className="flex min-w-0 items-start gap-3 py-3">
                      <Tags size={16} className="mt-0.5 shrink-0 text-of-muted" />
                      <div className="min-w-0">
                        <p className="text-xs font-medium">내부 분류</p>
                        <p className="mt-0.5 text-xs leading-5 text-of-muted">
                          {data.enabled
                            ? '정규화된 고객 태그와 정확 일치 필터를 사용할 수 있습니다.'
                            : '고객 태그 탐색을 숨기되 저장된 분류는 그대로 유지합니다.'}
                        </p>
                      </div>
                    </li>
                    <li className="flex min-w-0 items-start gap-3 py-3">
                      ...
                      />
                      <div className="min-w-0">
                        <p className="text-xs font-medium">진행 집계</p>
                        <p className="mt-0.5 text-xs leading-5 text-of-muted">
                          활성 상태에서는 사용자가 볼 수 있는 작업만 고객별 완료·지연 집계에
                          포함합니다.
                        </p>
                      </div>
                    </li>
                    <li className="flex min-w-0 items-start gap-3 py-3">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="mt-0.5 text-xs leading-5 text-of-muted">
                          정책을 꺼도 고객 기록, 태그와 작업 연결은 삭제되지 않으며 다시 켜면
                          복구됩니다.
                        </p>
                      </div>
                    </li>
                ...
                <section aria-labelledby="customers-audit-title">
                  <div className="border-b border-of-border-subtle pb-3">
                    <h2
                      id="customers-audit-title"
                  ...
                  <dl className="divide-y divide-of-border-subtle border-b border-of-border-subtle">
                    ...
                    <div className="flex items-start gap-2 py-3 text-xs leading-5 text-of-muted">
                      ...
                      <span>
                        ...
                        {data.enabled ? '고객 관리 사용 가능' : '고객 기능 안전 차단'}
                      </span>
                    </div>
                  </dl>
                </section>
