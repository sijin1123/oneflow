...
test('인앱 914x800 로그인은 승인 원본 기하를 최종 크기로 직접 렌더링한다', async ({ page }) => {
  ...
  expect(geometry.story).toMatchObject({ naturalWidth: 792, naturalHeight: 1086 })
...
test('로그인 원본 비주얼과 흐르는 협업 경로 강조를 유지한다', async ({ page }) => {
  ...
  expect(storyImage.currentSrc).toMatch(/oneflow-login-story-reference\.png/)
  expect(storyImage.naturalWidth).toBe(792)
  expect(storyImage.naturalHeight).toBe(1086)
  ...
  const headingTransform = await page.locator('.of-login-heading h2').evaluate((element) => {
    ...
    const panel = document.querySelector('.of-login-page')!.getBoundingClientRect()
    return { scaleX: matrix.a, x: matrix.e, y: matrix.f, panelWidth: panel.width }
  })
  const headingScale = headingTransform.panelWidth / 1448
  expect(headingTransform.scaleX).toBeCloseTo(0.99, 4)
  expect(headingTransform.x).toBeCloseTo(-4 * headingScale, 4)
  expect(headingTransform.y).toBeCloseTo(-1 * headingScale, 4)
...
test('로그인 reference-fit은 인앱 크기와 기준 크기에서 정수 raster를 선택한다', async ({ page }) => {
  ...
  expect(inAppGeometry.story.currentSrc).toMatch(/oneflow-login-story-reference\.png/)
  ...
  await expect(page.locator('.of-login-submit')).toHaveCSS(
    'background-image',
    'linear-gradient(100deg, rgb(86, 98, 241) 0%, rgb(87, 99, 242) 52%, rgb(88, 99, 240) 100%)',
  )
