...
test('Projects context sidebar는 primary·workspace·More·project 계층을 유지한다', async ({ page }) => {
  ...
  await expect(contextNav.getByText('워크스페이스', { exact: true })).toBeVisible()
  await expect(contextNav.getByRole('link', { name: '프로젝트', exact: true })).toHaveAttribute('aria-current', 'page')
  await expect(contextNav.getByRole('link', { name: '전체 작업' })).toHaveAttribute('href', '/work-items')
  ...
  await contextNav.getByText('더 보기', { exact: true }).click()
  await expect(contextNav.getByRole('link', { name: '리포트' })).toHaveAttribute('href', '/reports')
  await contextNav.getByRole('link', { name: '리포트' }).click()
  await expect(page).toHaveURL('/reports')
  await expect(contextNav.getByRole('link', { name: '리포트' })).toHaveAttribute('aria-current', 'page')

  await page.setViewportSize({ width: 390, height: 844 })
  await page.getByRole('button', { name: '사이드바 열기' }).click()
  await expect(page.getByRole('dialog', { name: '모바일 내비게이션' }).getByRole('link', { name: '리포트' })).toBeVisible()
  await expectNoHorizontalOverflow(page)
...
test('사이드바 접기와 내비게이션 개인화는 reload와 cross-tab에서 유지된다', async ({ page }) => {
  ...
  await expect(customizer.getByRole('button', { name: '시스템 상태 위로 이동' })).toBeFocused()
  ...
  await customizer.getByRole('button', { name: '개인 메모 위로 이동' }).click()
  await page.screenshot({
    path: '../../docs/screenshots/redevelopment/sidebar-personalization-ui/desktop-customize.png',
  })
...
test('Workspace popover가 실제 설정·멤버·로그아웃 흐름에 연결된다', async ({
  ...
  await page.screenshot({
    path: '../../docs/screenshots/redevelopment/shell-header-workspace-switcher-ui/desktop-popover.png',
  })
  await page.keyboard.press('Shift+Tab')
