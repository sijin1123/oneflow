...
test('관리자 webhook 표면이 endpoint와 delivery lifecycle을 실제 요청에 연결한다', async ({ page }) => {
  ...
  await page.route('**/api/v1/webhook-deliveries**', async (route) => {
    if (route.request().method() === 'POST') {
      const retried = {
        ...deliveries[0],
        id: 'del-2',
        event_type: 'oneflow.retry',
        status: 'succeeded',
        response_status: 204,
        error: null,
      }
      deliveries.unshift(retried)
  ...
  await expect(page.getByText('HTTP 500', { exact: false })).toBeVisible()
  ...
  await expectNoHorizontalOverflow(page)
  await page.screenshot({
    path: '../../docs/screenshots/redevelopment/webhook-delivery-ui/mobile.png',
    fullPage: true,
  })
