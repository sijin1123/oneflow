...
async function mockApi(page: Page, opts: { conflictOnPatch?: boolean } = {}) {
  ...
  await page.route(/\/api\/v1\/projects(?:\?.*)?$/, (route) => {
    ...
    const sortKey = url.searchParams.get('sort_key') ?? 'default'
    const sortDirection = url.searchParams.get('sort_direction') === 'desc' ? -1 : 1
    const offset = Number(url.searchParams.get('offset') ?? 0)
    const limit = Number(url.searchParams.get('limit') ?? 200)
    const scoped = projects.items.filter((item) => includeArchived || !item.archived_at)
    const summary = {
      projects: scoped.length,
      active: scoped.filter((item) => !item.archived_at).length,
      archived: scoped.filter((item) => item.archived_at).length,
      open_work_packages: scoped.reduce((sum, item) => sum + item.open_work_package_count, 0),
      overdue_work_packages: scoped.reduce((sum, item) => sum + item.overdue_count, 0),
      initiatives: scoped.reduce(
        (sum, item) => sum + item.initiatives.length + item.initiative_overflow,
        0,
      ),
    }
    const filtered = query
      ? scoped.filter((item) =>
          [item.key, item.name, item.description ?? '', ...item.initiatives.map(({ name }) => name)]
            .some((value) => value.toLocaleLowerCase('ko').includes(query)),
        )
      : [...scoped]
    if (sortKey !== 'default') {
      filtered.sort((left, right) => {
        const leftValue = left[sortKey as keyof ProjectListItem]
        const rightValue = right[sortKey as keyof ProjectListItem]
        const compared = typeof leftValue === 'number' && typeof rightValue === 'number'
          ? leftValue - rightValue
          : String(leftValue ?? '').localeCompare(String(rightValue ?? ''), 'ko')
        return compared !== 0
          ? compared * sortDirection
  ...
  await page.route('**/api/v1/projects/*/health-history**', (route) => {
    if (!new URL(route.request().url()).pathname.endsWith('/health-history')) {
      return route.fallback()
    }
    return route.fulfill({ json: { items: [], total: 0 } satisfies ProjectHealthHistoryList })
  })
...
test('프로젝트 작업 화면 제어가 보기·필터·분석·생성 흐름에 연결된다', async ({ page }) => {
  ...
  await controls.getByRole('button', { name: '작업 도구 더 보기' }).click()
  await expect(page.getByRole('menuitem', { name: '내보내기', exact: true })).toBeVisible()
  await expect(page.getByRole('menuitem', { name: '가져오기', exact: true })).toBeVisible()
  await page.keyboard.press('Escape')

  await openProjectWorkItemTool(page, '저장 뷰 관리')
  const savedViews = page.getByRole('region', { name: '저장 뷰 관리' })
  await expect(savedViews).toBeVisible()
  await savedViews.getByRole('button', { name: '저장 뷰 닫기' }).click()
  await expect(savedViews).toHaveCount(0)
...
test('Projects context sidebar는 disclosure·More panel·pin navigation을 유지한다', async ({ page }) => {
  ...
  await expect(morePanel).toHaveAttribute('data-phase', 'opening')
  await expect(morePanel).toHaveCSS('animation-name', 'of-sidebar-panel-opening')
  ...
  await expect(morePanel).toHaveAttribute('data-phase', 'open')
  await page.screenshot({
    path: '../../docs/screenshots/redevelopment/navigation-overlay-motion-ui/more-desktop.png',
  })
  ...
  await moreTrigger.click()
  await page.keyboard.press('Escape')
  await expect(morePanel).toHaveAttribute('data-phase', 'closing')
  await expect(morePanel).toHaveCSS('animation-name', 'of-sidebar-panel-closing')
  await expect(morePanel).toHaveCount(0)
  ...
  await expectNoHorizontalOverflow(page)
  await expect(mobileMorePanel).toHaveAttribute('data-phase', 'open')
  await page.screenshot({
    path: '../../docs/screenshots/redevelopment/navigation-overlay-motion-ui/more-mobile.png',
  })
  await mobileMorePanel.getByRole('button', { name: 'More 닫기' }).click()
  await expect(mobileMorePanel).toHaveAttribute('data-phase', 'closing')
  await expect(mobileMorePanel).toHaveCount(0)
})
...
test('프로젝트 행 메뉴는 즐겨찾기·링크·설정·소유자 보관을 실제 상태와 연결한다', async ({ page }) => {
  ...
  const viewerMenuTrigger = page.locator('button[aria-label="운영 개선 프로젝트 작업"]')
  await viewerMenuTrigger.focus()
  await viewerMenuTrigger.press('Enter')
  ...
  await expect(viewerMenu.getByRole('menuitem', { name: '설정' })).toBeFocused()
  await page.keyboard.press('Home')
  await expect(viewerMenu.getByRole('menuitem', { name: '즐겨찾기에 추가' })).toBeFocused()
  await page.keyboard.press('Escape')
  await expect(viewerMenu).toHaveCount(0)
  await expect(viewerMenuTrigger).toHaveAttribute('aria-expanded', 'false')
  await expect(viewerMenuTrigger).toBeFocused()

  await viewerMenuTrigger.click()
  await viewerMenu.getByRole('menuitem', { name: '즐겨찾기에 추가' }).click()
  ...
  await page.goto('/projects')
  const ownerMenuTrigger = page.locator('button[aria-label="OneFlow 도입 프로젝트 작업"]')
  await ownerMenuTrigger.focus()
  await ownerMenuTrigger.press('Enter')
  const ownerMenu = page.getByRole('menu', { name: 'OneFlow 도입 프로젝트 작업' })
  ...
  const mobileMenu = page.getByRole('menu', { name: '운영 개선 프로젝트 작업' })
  await expect(mobileMenu.getByRole('menuitem', { name: '설정' })).toBeVisible()
  await expectNoHorizontalOverflow(page)
  await page.screenshot({
...
test('프로젝트 디렉터리 카드와 목록 메뉴는 즐겨찾기·복사·권한·보관 복원을 실제 상태와 연결한다', async ({ page }) => {
  test.setTimeout(90_000)
  await page.addInitScript(() => {
    Object.defineProperty(navigator, 'clipboard', {
      configurable: true,
      value: {
        writeText: async (text: string) => window.localStorage.setItem('__copied_directory_project_link', text),
      },
    })
  })
  await mockApi(page)
  const viewerProject: ProjectListItem = {
    ...project,
    ...projectRollups,
    id: '99999999-9999-4999-8999-999999999998',
    key: 'OPS',
  ...
  await page.route(/\/api\/v1\/projects(?:\?.*)?$/, (route) => {
    const owner = { ...project, ...projectRollups, archived_at: ownerArchived ? '2026-07-21T00:00:00Z' : null }
    const includeArchived = new URL(route.request().url()).searchParams.get('include_archived') === 'true'
    const items = ownerArchived && !includeArchived ? [viewerProject] : [owner, viewerProject]
    return route.fulfill({ json: { items, total: items.length } satisfies ProjectList })
  })
  await page.route(`**/api/v1/projects/${viewerProject.id}/members`, (route) => route.fulfill({
    json: {
      items: [{ user_id: 'me-1', email: 'dev@oneflow.local', display_name: 'Dev User', role: 'viewer' }],
  ...
  await page.route(`**/api/v1/projects/${project.id}/archive`, (route) => {
    ownerArchived = true
    return route.fulfill({ json: { ...project, archived_at: '2026-07-21T00:00:00Z' } })
  })
  await page.route(`**/api/v1/projects/${project.id}/unarchive`, (route) => {
    ownerArchived = false
    return route.fulfill({ json: { ...project, archived_at: null } })
  })

  await page.goto('/projects')
  const directory = page.getByRole('list', { name: '프로젝트 디렉터리' })
  const ownerCard = directory.getByRole('listitem').filter({ hasText: 'OneFlow 도입' })
  const favorite = ownerCard.getByRole('button', { name: 'OneFlow 도입 즐겨찾기에 추가' })
  await favorite.click()
  await expect(ownerCard.getByRole('button', { name: 'OneFlow 도입 즐겨찾기 해제' }))
    .toHaveAttribute('aria-pressed', 'true')
  const contextNav = page.getByRole('navigation', { name: 'Projects 컨텍스트 내비게이션' })
  await expect(contextNav.getByRole('link', { name: /OneFlow 도입/ }).getByLabel('즐겨찾기')).toBeVisible()

  const ownerMenuTrigger = ownerCard.getByRole('button', { name: 'OneFlow 도입 프로젝트 카드 작업' })
  await ownerMenuTrigger.focus()
  await ownerMenuTrigger.press('Enter')
  ...
  await page.keyboard.press('Escape')
  await expect(ownerMenu).toHaveCount(0)
  await expect(ownerMenuTrigger).toBeFocused()

  const viewerCard = directory.getByRole('listitem').filter({ hasText: '운영 개선' })
  ...
  const includeArchived = page.getByRole('checkbox', { name: '보관 포함' })
  await includeArchived.click({ noWaitAfter: true })
  ...
  await page.getByRole('menuitem', { name: '프로젝트 보관' }).click()
  await archiveRequest
  await expect(page.getByRole('status', { name: '프로젝트 작업 결과' })).toContainText('프로젝트를 보관했습니다')
  await expect(ownerCard.getByText('보관됨')).toBeVisible()

  await ownerCard.getByRole('button', { name: 'OneFlow 도입 프로젝트 카드 작업' }).click()
  const restoreRequest = page.waitForRequest(
    (request) => request.method() === 'POST' && request.url().endsWith(`/projects/${project.id}/unarchive`),
  )
  page.once('dialog', (dialog) => void dialog.accept())
  await page.getByRole('menuitem', { name: '프로젝트 복원' }).click()
  await restoreRequest
  await expect(page.getByRole('status', { name: '프로젝트 작업 결과' })).toContainText('프로젝트를 복원했습니다')

  await page.getByRole('button', { name: '목록 보기' }).click()
  await expect(directory.getByRole('button', { name: 'OneFlow 도입 프로젝트 목록 작업' })).toBeVisible()
  await page.setViewportSize({ width: 390, height: 844 })
  await directory.getByRole('button', { name: 'OneFlow 도입 프로젝트 목록 작업' }).click()
  await expect(page.getByRole('menu', { name: 'OneFlow 도입 프로젝트 목록 작업' })).toBeVisible()
  await expectNoHorizontalOverflow(page)
  await page.screenshot({
...
test('모바일 Customize navigation은 손상된 저장값을 복구하고 drawer에서 동작한다', async ({ page }) => {
  ...
  const customizer = page.getByRole('dialog', { name: '내비게이션 사용자 지정' })
  await expect(customizer).toHaveAttribute('data-phase', 'open')
  const customizerBox = await customizer.boundingBox()
  expect(customizerBox?.x).toBeGreaterThanOrEqual(0)
...
test('AI workspace는 Ask 요약과 검토 후 Build 생성을 실제 API에 연결한다', async ({ page }) => {
  ...
  await page.route(`**/api/v1/work-packages/${wpA.id}/summary`, async (route) => {
    ...
    await route.fulfill({
      json: {
        work_package_id: wpA.id,
        summary: 'API 구현의 현재 범위와 남은 검증을 정리했습니다.',
        provider: 'test',
      },
    })
  })
  const createdWorkPackage: WorkPackage = {
    ...wpB,
  ...
  await page.route(`**/api/v1/projects/${project.id}/work-packages`, async (route) => {
    ...
    if (createAttempts === 1) {
      await route.fulfill({ status: 503, json: { detail: 'temporary build failure' } })
      return
    }
    await route.fulfill({ status: 201, json: createdWorkPackage })
  })
  await page.goto('/ai')
  ...
  await expect(aiNav.getByRole('link', { name: 'OneFlow AI' })).toHaveAttribute('href', '/ai')
  ...
  await page.getByLabel('작업 제목').fill(createdWorkPackage.subject)
  await page.getByLabel('우선순위').selectOption('urgent')
  await page.getByLabel('기한').fill('2026-08-01')
  await page.getByRole('button', { name: '작업 확인' }).click()
  const review = page.getByRole('dialog', { name: '이 작업을 만들까요?' })
  await expect(review).toBeVisible()
  await expect(review.getByText(createdWorkPackage.subject)).toBeVisible()
  await review.getByRole('button', { name: '작업 만들기' }).click()
  await expect(review.getByRole('alert')).toContainText('temporary build failure')
  await review.getByRole('button', { name: '작업 만들기' }).click()
  await expect(review).toHaveCount(0)
  await expect(page.getByRole('status')).toContainText('작업을 만들었습니다')
  await expect(page.getByRole('link', { name: '작업 열기' })).toHaveAttribute('href', `/projects/${project.id}/work-packages?wp=${createdWorkPackage.id}`)
  ...
  const quickDockButton = page.getByRole('button', { name: '빠른 도구 열기' })
  const [sendBox, quickDockBox] = await Promise.all([sendButton.boundingBox(), quickDockButton.boundingBox()])
  ...
  await expectNoHorizontalOverflow(page)
  await page.screenshot({ path: '../../docs/screenshots/redevelopment/ai-ask-build-ui/mobile-navigation.png' })
})

test('AI workspace는 Ask 조회 오류와 Build 프로젝트 상태를 서로 격리한다', async ({ page }) => {
  await mockApi(page)
  await page.route('**/api/v1/capabilities', (route) => route.fulfill({ json: { ai_summary_enabled: true } }))
  let myWorkHealthy = false
  await page.route('**/api/v1/me/work', async (route) => {
    if (!myWorkHealthy) {
      await route.fulfill({ status: 500, json: { detail: 'my work unavailable' } })
      return
    }
    await route.fulfill({
      json: {
  ...
  const conversation = page.getByLabel('AI 대화')
  await expect(conversation.getByRole('alert')).toContainText('my work unavailable')
  await page.getByRole('tab', { name: 'Build' }).click()
  await expect(page.getByLabel('프로젝트')).toHaveValue(project.id)
  await expect(page.getByLabel('작업 제목')).toBeEnabled()

  await page.getByRole('tab', { name: 'Ask' }).click()
  myWorkHealthy = true
  await conversation.getByRole('button', { name: '다시 시도' }).click()
  await expect(page.getByLabel('작업 범위')).toHaveValue(wpA.id)
...
test('Workspace Views가 Board·Table·scope·filter·sort를 같은 결과 계약으로 연결한다', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: '보드 뷰 구현', exact: true })).toHaveCount(0)
  ...
  await expect(page.getByRole('button', { name: '보드 뷰 구현', exact: true })).toBeVisible()
  await expect(page.getByRole('button', { name: '워크패키지 API 구현', exact: true })).toHaveCount(0)
...
test('보드 카드 액션 메뉴가 링크·복제·이동·전체 페이지 흐름을 연결한다', async ({ page }) => {
  ...
  const openCardActions = async () => {
    ...
    await card.hover()
    const trigger = card.getByRole('button', { name: '워크패키지 API 구현 카드 작업' })
  ...
  await page.getByRole('menuitem', { name: '상세 드로어 열기' }).click()
  await expect(page).toHaveURL(new RegExp(`wp=${wpA.id}`))
  let drawer = page.getByRole('dialog')
  await expect(drawer).toContainText('워크패키지 API 구현')
  await drawer.getByRole('button', { name: '닫기' }).click()
  await expect(page).not.toHaveURL(/wp=/)
...
test('모바일 보드 카드 액션 메뉴는 hover 없이 열리고 폭을 넘지 않는다', async ({ page }) => {
  ...
  const openDetail = menu.getByRole('menuitem', { name: '상세 드로어 열기' })
  const openMove = menu.getByRole('menuitem', { name: '이동 패널 열기' })
...
test('뷰어 보드 카드 액션 메뉴는 쓰기 액션 없이 읽기 전용으로 표시된다', async ({ page }) => {
  ...
  await trigger.click()
  const menu = page.getByRole('menu')
  await expect(menu.getByRole('menuitem', { name: '읽기 전용' })).toBeVisible()
  await expect(page.getByRole('menuitem', { name: '복제' })).toHaveCount(0)
  await expect(page.getByRole('menuitem', { name: '이동 패널 열기' })).toHaveCount(0)
  await expect(menu.getByRole('menuitem', { name: '상세 드로어 열기' })).toBeFocused()
  await page.keyboard.press('ArrowUp')
...
test('작업 상세 압축 헤더가 제목 저장과 핵심 명령을 한 표면에서 연결한다', async ({ page }) => {
  await mockApi(page)
  await page.goto(`/projects/${project.id}/work-packages`)
  await page.getByRole('button', { name: '워크패키지 API 구현', exact: true }).click()
  const drawer = page.getByRole('dialog', { name: '워크패키지 API 구현' })
  const toolbar = drawer.getByRole('toolbar', { name: '작업 명령' })

  await expect(drawer.getByText(/v0 · Dev User 생성/)).toBeVisible()
  await expect(toolbar.getByRole('button', { name: '워치' })).toBeVisible()
  await expect(toolbar.getByRole('button', { name: '복제' })).toBeVisible()
  await expect(toolbar.getByRole('button', { name: '이동' })).toBeVisible()
  await expect(toolbar.getByRole('link', { name: '전체 페이지' })).toBeVisible()

  const subject = drawer.getByLabel('제목')
  const patchRequest = page.waitForRequest(
    (request) => request.method() === 'PATCH' && request.url().endsWith(`/work-packages/${wpA.id}`),
  )
  await subject.fill('압축 헤더 제목 수정')
  await subject.blur()
  expect((await patchRequest).postDataJSON()).toEqual({
    expected_version: 0,
    subject: '압축 헤더 제목 수정',
  })
  await expect(subject).toHaveValue('압축 헤더 제목 수정')
  await expect(page.getByRole('dialog', { name: '압축 헤더 제목 수정' })).toBeVisible()
  await page.screenshot({
    path: '../../docs/screenshots/redevelopment/detail-header-ui/desktop.png',
    fullPage: true,
  })
})
...
test('활동과 댓글은 저장된 actor 이름과 이미지 snapshot을 렌더링한다', async ({ page }) => {
  await mockApi(page)
  const png = Buffer.from(
    'iVBORw0KGgoAAAANSUhEUgAAAAIAAAACCAIAAAD91JpzAAAAEklEQVR4nGPkndLBwMDAxAAGAA2bAS37E8jFAAAAAElFTkSuQmCC',
    'base64',
  )
  const version = '11111111-1111-4111-8111-111111111119'
  const commentImage = `/api/v1/work-packages/${wpA.id}/comments/history-comment/actor-image?version=${version}`
  const activityImage = `/api/v1/work-packages/${wpA.id}/activities/history-activity/actor-image?version=${version}`
  await page.route(`**/api/v1/work-packages/${wpA.id}/activities**`, (route) =>
    route.fulfill({
      json: {
        items: [
          {
            ...
            work_package_id: wpA.id,
            actor_id: null,
            actor_name: '저장된 활동 사용자',
            actor_profile_image_url: activityImage,
            action: 'created',
            field: null,
            old_value: null,
            new_value: null,
        ...
        items: [
          {
            root: {
              ...
              parent_id: null,
              author_id: null,
              author_name: '저장된 댓글 사용자',
              author_profile_image_url: commentImage,
              body: '과거 댓글 본문',
              mentions: null,
              reactions: [],
  ...
  // Playwright evaluates newer handlers first, so image routes follow the
  // broader activity endpoint mock above.
  await page.route(`**${commentImage}`, (route) =>
    route.fulfill({ body: png, contentType: 'image/png' }),
  )
  await page.route(`**${activityImage}`, (route) =>
    route.fulfill({ body: png, contentType: 'image/png' }),
  )
  ...
  await drawer.getByRole('tab', { name: '활동' }).click()
  const timeline = drawer.getByRole('list', { name: '활동 타임라인' })
  await expect(timeline.getByLabel('저장된 활동 사용자').locator('img')).toBeVisible()
  await expect(timeline.getByLabel('저장된 댓글 사용자').locator('img')).toBeVisible()
  await expect(timeline).toContainText('저장된 활동 사용자')
  await expect(timeline).toContainText('저장된 댓글 사용자')
  await expectNoHorizontalOverflow(page)
  await page.screenshot({
    path: '../../docs/screenshots/redevelopment/activity-identity-avatars-ui/desktop.png',
    fullPage: true,
  })
  await page.setViewportSize({ width: 390, height: 844 })
  await expectNoHorizontalOverflow(page)
  await page.screenshot({
    path: '../../docs/screenshots/redevelopment/activity-identity-avatars-ui/mobile.png',
    fullPage: true,
  })
})
...
test('작업 행 액션 메뉴가 링크·복제·이동·전체 페이지 흐름을 연결한다', async ({ page }) => {
  ...
  const openRowActions = async () => {
    ...
    await row.hover()
    const trigger = row.getByRole('button', { name: '워크패키지 API 구현 행 작업' })
    await expect(trigger).toBeVisible()
    await trigger.click()
  }
...
test('모바일 작업 행 액션 메뉴는 hover 없이 열리고 폭을 넘지 않는다', async ({ page }) => {
  ...
  const trigger = page
    ...
    .getByRole('button', { name: '워크패키지 API 구현 행 작업' })
  await expect(trigger).toBeVisible()
  await trigger.click()
  const menu = page.getByRole('menu')
  const openDetail = menu.getByRole('menuitem', { name: '상세 드로어 열기' })
...
test('타임라인 항목 액션 메뉴가 링크·복제·이동·전체 페이지 흐름을 연결한다', async ({ page }) => {
  ...
  await expect(detailDrawer).toContainText('워크패키지 API 구현')
  await detailDrawer.getByRole('button', { name: '닫기' }).click()
  await expect(page).not.toHaveURL(/wp=/)
...
test('모바일 타임라인 항목 액션 메뉴는 hover 없이 열리고 폭을 넘지 않는다', async ({ page }) => {
  ...
  await expect(action).toHaveAttribute('aria-controls', `timeline-actions-${wpA.id}`)
  ...
  await action.click()
  await expect(menu).toBeVisible()
  await page.getByRole('heading', { name: '타임라인' }).click()
  await expect(menu).toBeHidden()
...
test('뷰어 타임라인 항목 액션 메뉴는 쓰기 액션 없이 읽기 전용으로 표시된다', async ({ page }) => {
  ...
  await expect(menu).toBeHidden()
  await expect(
    page.getByRole('button', { name: '워크패키지 API 구현 타임라인 항목 작업' }),
  ).toBeFocused()
})
...
test('AI workspace가 켜진 AI 요약 기능을 보이는 작업 상세로 연결한다', async ({ page }) => {
  ...
  await expect(drawer.getByRole('button', { name: '생성', exact: true })).toBeVisible()
})
...
test('모듈 작업 메뉴가 모바일 폭에서 잘리고 넘치지 않는다', async ({ page }) => {
  ...
  await page.goto(`/projects/${project.id}/modules`)
  const trigger = page.getByRole('button', { name: '인증 모듈 모듈 작업' })
  await trigger.focus()
  await trigger.press('Enter')
  const menu = page.getByRole('menu', { name: '인증 모듈 모듈 작업' })
  await expect(menu).toBeVisible()
  await expect(menu.getByRole('menuitem', { name: '작업 목록 열기' })).toBeFocused()
  await page.keyboard.press('ArrowUp')
  await expect(menu.getByRole('menuitem', { name: '삭제' })).toBeFocused()
  await page.keyboard.press('Home')
  await expect(menu.getByRole('menuitem', { name: '작업 목록 열기' })).toBeFocused()
  await page.keyboard.press('ArrowDown')
  await expect(menu.getByRole('menuitem', { name: '참여자 관리' })).toBeFocused()
  await page.keyboard.press('End')
  await expect(menu.getByRole('menuitem', { name: '삭제' })).toBeFocused()
  await page.keyboard.press('Escape')
  ...
  await trigger.click()
  await page.getByText('모듈').first().click()
  await expect(menu).toHaveCount(0)
...
test('워처 구독 표면은 모바일에서 알림 단서와 참여자를 유지한다', async ({ page }) => {
  ...
  await section.getByRole('button', { name: '워처 4명 보기' }).click()
  const details = page.getByRole('region', { name: '워처 상세' })
  await expect(details.getByText('상태 변경')).toBeVisible()
  await expect(details.getByText('댓글', { exact: true })).toBeVisible()
  await expect(details.getByText('담당자', { exact: true })).toBeVisible()
  await expect(details.getByText('Dev User')).toBeVisible()
  await expect(details.getByText('Ops Lead')).toBeVisible()
  await expect(details.getByText('QA Owner')).toBeVisible()
  await expect(details.getByText('외 1명')).toBeVisible()
  await expectNoHorizontalOverflow(page)
  await page.screenshot({
...
test('워크스페이스 로고는 미리보기·저장·shell 반영·삭제가 하나의 revision으로 동작한다', async ({ page }) => {
  ...
  let profile = {
    ...
    logo_content_type: null as string | null,
    logo_filename: null as string | null,
    logo_width: null as number | null,
    logo_height: null as number | null,
    logo_byte_size: null as number | null,
    updated_by_user_id: null as string | null,
    updated_by_name: null as string | null,
    updated_at: '2026-07-01T00:00:00Z',
  }
  ...
  await page.route('**/api/v1/admin/workspace/profile', (route) =>
    route.fulfill({ json: profile, headers: { ETag: `"${profile.revision}"` } }),
  )
  await page.route('**/api/v1/admin/workspace/logo', async (route) => {
    if (route.request().method() === 'PUT') {
      expect(route.request().headers()['if-match']).toBe('"1"')
      expect(route.request().headers()['content-type']).toBe('image/png')
      expect(route.request().headers()['x-file-name']).toBe('oneflow-mark.png')
      expect(route.request().postDataBuffer()).toEqual(logoPng)
      profile = {
        ...profile,
        revision: 2,
        logo_url: '/api/v1/workspace/logo?revision=2',
        logo_content_type: 'image/png',
        logo_filename: 'oneflow-mark.png',
        logo_width: 2,
        logo_height: 2,
        logo_byte_size: logoPng.length,
        updated_by_user_id: 'me-1',
        updated_by_name: 'Dev User',
  ...
  await page.getByLabel('워크스페이스 로고 파일').setInputFiles({
    name: 'oneflow-mark.png',
    mimeType: 'image/png',
    buffer: logoPng,
  })
  await expect(page.getByAltText('선택한 로고 미리보기')).toBeVisible()
  await expect(page.getByText('선택됨: oneflow-mark.png')).toBeVisible()
  await page.getByRole('button', { name: '로고 저장' }).click()
  await expect(page.getByText('oneflow-mark.png · 2×2 px')).toBeVisible()
  await expect(page.getByText('revision 2')).toBeVisible()
  await expect(
  ...
  await expect(workspaceMenu.getByAltText('OneFlow 로고')).toBeVisible()
  await page.screenshot({
    path: '../../docs/screenshots/redevelopment/workspace-branding-ui/desktop.png',
    fullPage: true,
  })
  await page.keyboard.press('Escape')

  await page.setViewportSize({ width: 390, height: 844 })
  await expect(page.locator('header img[alt="OneFlow 로고"]:visible')).toBeVisible()
  await expectNoHorizontalOverflow(page)
  await page.screenshot({
    path: '../../docs/screenshots/redevelopment/workspace-branding-ui/mobile.png',
    fullPage: true,
  })
  ...
  await expect(page.getByLabel('OneFlow 로고 기본값').first()).toBeVisible()
})

test('워크스페이스 로고는 잘못된 파일을 막고 충돌 시 선택을 보존한다', async ({ page }) => {
  await mockApi(page)
  const logoPng = Buffer.from(
    'iVBORw0KGgoAAAANSUhEUgAAAAIAAAACCAIAAAD91JpzAAAAEklEQVR4nGPkndLBwMDAxAAGAA2bAS37E8jFAAAAAElFTkSuQmCC',
    'base64',
  ...
  const profile = {
    ...
    name: 'OneFlow',
    revision: 1,
    logo_url: '/api/v1/workspace/logo?version=00000000-0000-0000-0000-000000000002',
    logo_content_type: 'image/png',
    logo_filename: 'old-mark.png',
    logo_width: 2,
    logo_height: 2,
    logo_byte_size: logoPng.length,
  ...
  await page.route('**/api/v1/workspace/profile', (route) => route.fulfill({ json: profile }))
  await page.route('**/api/v1/workspace/logo**', (route) => route.fulfill({ status: 404 }))
  await page.route('**/api/v1/admin/workspace/profile', (route) =>
    route.fulfill({ json: profile, headers: { ETag: '"1"' } }),
  )
  await page.route('**/api/v1/admin/workspace/logo', (route) =>
    route.fulfill({
      status: 412,
      json: { detail: { code: 'stale_revision', current_revision: 2 } },
      headers: { ETag: '"2"' },
  ...
  await page.goto('/admin/general')
  await expect(
    page.getByRole('button', { name: '워크스페이스 전환' }).getByLabel('OneFlow 로고 기본값'),
  ).toBeVisible()
  const input = page.getByLabel('워크스페이스 로고 파일')
  await input.setInputFiles({ name: 'logo.txt', mimeType: 'text/plain', buffer: Buffer.from('x') })
  await expect(page.getByRole('alert')).toContainText('PNG, JPEG 또는 WebP')
  await expect(page.getByRole('button', { name: '로고 저장' })).toHaveCount(0)
  ...
  await expect(page.getByRole('alert')).toContainText('2 MiB 이하')
  await expect(page.getByRole('button', { name: '로고 저장' })).toHaveCount(0)

  await input.setInputFiles({ name: 'replacement.png', mimeType: 'image/png', buffer: logoPng })
  await page.getByRole('button', { name: '로고 저장' }).click()
  await expect(page.getByRole('alert')).toContainText('다른 관리자가 먼저 변경했습니다')
  await expect(page.getByText(/선택됨: replacement\.png/)).toBeVisible()
  await expect(page.getByRole('button', { name: '로고 저장' })).toBeVisible()
})
...
test('인테이크 판정 이력은 펼칠 때 지연 조회하고 모바일에서도 전이 흐름을 유지한다', async ({
  ...
  const avatarPng = Buffer.from(
    'iVBORw0KGgoAAAANSUhEUgAAABAAAAAQCAIAAACQkWg2AAAAI0lEQVR4nGNUX5fAQApgIkk1w6gG4gATkergYFQDMYDkUAIA4+oBVRXfMoQAAAAASUVORK5CYII=',
    'base64',
  )
  const actorImage = `/api/v1/projects/${project.id}/intake/it-history/history/decision-2/actor-image?version=22222222-2222-4222-8222-222222222222`
  let historyRequests = 0
  let actorImageRequests = 0
  await page.route(
          ...
          items: [
            {
              ...
              decided_by_profile_image_url: actorImage,
            ...
            },
            {
              ...
              decided_by: 'me-1',
              decided_by_name: 'Dev User',
              decided_by_profile_image_url: null,
  ...
  await page.route('**/api/v1/projects/*/intake/*/history/*/actor-image**', (route) => {
    actorImageRequests += 1
    return route.fulfill({ body: avatarPng, contentType: 'image/png' })
  })
  await page.route(`**/api/v1/projects/${project.id}/intake`, (route) =>
    route.fulfill({
      json: {
  ...
  const actorAvatar = timeline.locator('li').first().locator('img')
  await expect(actorAvatar).toBeVisible()
  await expect.poll(() => actorImageRequests).toBe(1)
  await expect
    .poll(() => actorAvatar.evaluate((image: HTMLImageElement) => image.naturalWidth))
    .toBeGreaterThan(0)
...
test('보드에서 카드를 드래그해 옮기면 상태 PATCH가 간다', async ({ page }) => {
  await mockApi(page)
  await page.goto(`/projects/${project.id}/board`)
  const card = page.getByRole('button', { name: /워크패키지 API 구현 작업/ })
  ...
  await page.getByLabel('스윔레인 기준').selectOption('priority')
  const laneCard = page.getByRole('button', { name: /워크패키지 API 구현 작업/ })
...
test('사이클 작업 메뉴가 모바일 폭에서 잘리고 넘치지 않는다', async ({ page }) => {
  ...
  await page.goto(`/projects/${project.id}/cycles`)
  const trigger = page.getByRole('button', { name: '7월 스프린트 사이클 작업' })
  await trigger.focus()
  await trigger.press('Enter')
  ...
  await expect(menu.getByRole('menuitem', { name: '작업 목록 열기' })).toBeFocused()
  await page.keyboard.press('ArrowDown')
  await expect(menu.getByRole('menuitem', { name: '번다운 보기' })).toBeFocused()
  await page.keyboard.press('End')
  await expect(menu.getByRole('menuitem', { name: '삭제' })).toBeFocused()
  await page.keyboard.press('Home')
  await expect(menu.getByRole('menuitem', { name: '작업 목록 열기' })).toBeFocused()
  ...
  await trigger.click()
  await page.getByText('사이클').first().click()
  await expect(menu).toHaveCount(0)

  await trigger.click()
  await menu.getByRole('menuitem', { name: '번다운 보기' }).click()
  await expect(page.getByText('번다운을 불러오지 못했습니다.')).toBeVisible()
  await expect(menu).toHaveCount(0)
...
test('상태 관리 액션 메뉴에서 라벨을 바꾸고 순서를 조정한다', async ({ page }) => {
  ...
  const statusMenuTrigger = page.getByLabel('todo 상태 작업')
  const statusMenu = page.getByRole('menu', { name: 'todo 상태 작업 메뉴' })
  const editStatus = page.getByLabel('todo 상태 편집')
  const moveStatusDown = page.getByLabel('todo 아래로')
  const closeStatusMenu = statusMenu.getByRole('menuitem', { name: '닫기' })

  await statusMenuTrigger.focus()
  await statusMenuTrigger.press('Enter')
  await expect(editStatus).toBeFocused()
  await page.keyboard.press('ArrowDown')
  await expect(moveStatusDown).toBeFocused()
  await page.keyboard.press('ArrowDown')
  await expect(closeStatusMenu).toBeFocused()
  await page.keyboard.press('ArrowDown')
  await expect(editStatus).toBeFocused()
  await page.keyboard.press('ArrowUp')
  await expect(closeStatusMenu).toBeFocused()
  ...
  await expect(statusMenuTrigger).toBeFocused()

  await statusMenuTrigger.click()
  await expect(editStatus).toBeFocused()
  await page.getByText('워크플로우 상태').click()
  await expect(statusMenu).toHaveCount(0)
...
test('CSV 가져오기: dry-run 미리보기 후 실행하고 실패 행을 격리한다', async ({ page }) => {
  ...
  await page.goto(`/projects/${project.id}/work-packages`)
  await openProjectWorkItemTool(page, '가져오기')
...
test('트리 항목 액션 메뉴가 링크·복제·이동·전체 페이지 흐름을 연결한다', async ({ page }) => {
  await page.addInitScript(() => {
    Object.defineProperty(navigator, 'clipboard', {
      configurable: true,
      value: {
        writeText: async (text: string) => window.localStorage.setItem('__copied_tree_item_link', text),
      },
    })
  })
  await mockApi(page)
...
test('모바일 트리 항목 액션 메뉴는 hover 없이 열리고 폭을 넘지 않는다', async ({ page }) => {
  ...
  await expect(menu.getByRole('menuitem', { name: '상세 드로어 열기' })).toBeFocused()
  await page.keyboard.press('ArrowUp')
  await expect(menu.getByRole('menuitem', { name: '이동 패널 열기' })).toBeFocused()
  const box = await menu.boundingBox()
  expect(box).not.toBeNull()
  expect(box!.x).toBeGreaterThanOrEqual(0)
  expect(box!.x + box!.width).toBeLessThanOrEqual(390)
...
test('캘린더 항목 액션 메뉴가 링크·복제·이동·전체 페이지 흐름을 연결한다', async ({ page }) => {
  ...
  const openItemActions = async () => {
    ...
    await item.hover()
    const trigger = item.getByRole('button', { name: '워크패키지 API 구현 캘린더 항목 작업' })
    await expect(trigger).toBeVisible()
    await trigger.click()
  }
...
test('모바일 캘린더 항목 액션 메뉴는 hover 없이 열리고 폭을 넘지 않는다', async ({ page }) => {
  ...
  const menu = page.getByRole('menu')
  await expect(menu.getByRole('menuitem', { name: '상세 드로어 열기' })).toBeFocused()
  await expect(menu.getByRole('menuitem', { name: /링크 복사/ })).toBeVisible()
  await page.keyboard.press('ArrowUp')
  await expect(menu.getByRole('menuitem', { name: '이동 패널 열기' })).toBeFocused()
  const overflow = await page.evaluate(() => document.documentElement.scrollWidth - window.innerWidth)
  expect(overflow).toBeLessThanOrEqual(1)
...
test('알림 벨이 미확인 개수를 보여주고 모두 읽음을 보낸다', async ({ page }) => {
    ...
    items: [
      {
        ...
        work_package_subject: '워크패키지 API 구현',
        actor_name: 'Dev User',
        actor_profile_image_url: `/api/v1/me/notifications/n1/actor-image?version=${actorVersion}`,
        read: false,
        created_at: '2026-07-05T09:00:00Z',
      },
      {
  ...
  const actorImage = drawer.locator('img[src*="/notifications/n1/actor-image"]')
  await expect(actorImage).toHaveCount(1)
  await expect.poll(() => actorImage.evaluate((image: HTMLImageElement) => image.naturalWidth)).toBe(2)
  // System due alert (Pass 40): actor-less wording.
  await expect(drawer.getByText(/기한이 지났습니다/)).toBeVisible()
  await expect(drawer.getByLabel('시스템 알림')).toHaveCount(1)
  await page.screenshot({
    path: '../../docs/screenshots/redevelopment/inbox-notification-avatars-ui/sheet-desktop.png',
  })
...
test('공통 Sheet는 닫힘 동작과 포커스 복귀를 같은 전환에서 유지한다', async ({ page }) => {
  await mockApi(page)
  await page.route('**/api/v1/me/notifications', (route) =>
    route.fulfill({ json: { items: [], total: 0, unread: 0 } }),
  )
  ...
  await page.goto(`/projects/${project.id}/work-packages`)
  const trigger = page.getByRole('button', { name: '알림' })
  await trigger.click()
  ...
  await expect(sheet).toHaveCSS('animation-name', 'of-sheet-content-opening')

  await sheet.getByRole('button', { name: '닫기' }).evaluate((button) => {
    ;(button as HTMLButtonElement).click()
  })

  expect(await sheet.getAttribute('data-state')).toBe('closed')
  expect(await overlay.getAttribute('data-state')).toBe('closed')
  expect(await sheet.evaluate((element) => getComputedStyle(element).animationName)).toBe(
    'of-sheet-content-closing',
  )
  expect(await overlay.evaluate((element) => getComputedStyle(element).animationName)).toBe(
    'of-sheet-overlay-closing',
  )
...
test('인박스는 알림 센터를 모바일에서도 정리된 표면으로 보여준다', async ({ page }) => {
  ...
  const inboxActorImage = page.locator('img[src*="/notifications/n1/actor-image"]')
  await expect(inboxActorImage).toHaveCount(1)
  await expect
    .poll(() => inboxActorImage.evaluate((image: HTMLImageElement) => image.naturalWidth))
    .toBe(2)
  await expectNoHorizontalOverflow(page)
  await page.screenshot({
    path: '../../docs/screenshots/redevelopment/inbox-notification-avatars-ui/inbox-mobile.png',
    fullPage: true,
  })
...
test('커맨드 팔레트 단축키는 편집 필드를 침범하지 않는다', async ({ page }) => {
  ...
  await page.getByRole('button', { name: '워크패키지 API 구현', exact: true }).focus()
  await page.keyboard.press('Control+K')
...
test('AI 요약은 실제 POST로 생성하고 재생성 실패 시 마지막 성공값을 보존해 재시도한다', async ({ page }) => {
  await mockApi(page)
  await page.route('**/api/v1/capabilities', (route) =>
    route.fulfill({ json: { ai_summary_enabled: true } }),
  )
  ...
  await page.route(`**/api/v1/work-packages/${wpA.id}/summary`, (route) => {
    attempts += 1
    if (attempts === 2) {
      return route.fulfill({ status: 503, json: { detail: '요약 공급자를 사용할 수 없습니다.' } })
    }
    return route.fulfill({
      json: {
        work_package_id: wpA.id,
        summary: attempts === 1
          ? "'워크패키지 API 구현'은 유형 '작업', 상태 '할 일'인 작업입니다."
          : '갱신된 작업 요약입니다.',
  ...
  await page.getByRole('button', { name: '워크패키지 API 구현', exact: true }).click()
  const drawer = page.getByRole('dialog')
  const section = drawer.getByRole('region', { name: 'AI 요약' })
  await expect(section).toBeVisible()
  await expect(section.getByText('아직 생성된 요약이 없습니다.')).toBeVisible()

  await section.getByRole('button', { name: '생성', exact: true }).click()
  await expect(section.getByText(/유형 '작업', 상태 '할 일'/)).toBeVisible()
  await section.getByRole('button', { name: '새로 고침', exact: true }).click()
  await expect(section.getByRole('alert')).toContainText('요약 공급자를 사용할 수 없습니다.')
  await expect(section.getByText(/유형 '작업', 상태 '할 일'/)).toBeVisible()
  await section.getByRole('button', { name: '다시 시도', exact: true }).click()
  await expect(section.getByText('갱신된 작업 요약입니다.')).toBeVisible()
  expect(attempts).toBe(3)
  await page.mouse.move(1, 1)
  await page.screenshot({
    path: '../../docs/screenshots/redevelopment/detail-ai-summary-ui/desktop.png',
    fullPage: true,
  })
})

test('AI 요약은 capability가 꺼져 있으면 상세에 노출되지 않는다', async ({ page }) => {
  await mockApi(page)
  await page.goto(`/projects/${project.id}/work-packages/${wpA.id}`)
  await expect(page.getByRole('region', { name: 'AI 요약' })).toHaveCount(0)
})

test('AI 요약은 작업 전환 시 이전 결과를 누출하지 않고 모바일 계층을 유지한다', async ({ page }) => {
  await mockApi(page)
  await page.route('**/api/v1/capabilities', (route) =>
    route.fulfill({ json: { ai_summary_enabled: true } }),
  )
  await page.route(`**/api/v1/work-packages/${wpB.id}`, (route) =>
    route.fulfill({ json: wpB }),
  )
  await page.route(`**/api/v1/work-packages/${wpB.id}/relations`, (route) =>
    route.fulfill({ json: { items: [], total: 0 } }),
  )
  await page.route(`**/api/v1/work-packages/${wpB.id}/time-entries`, (route) =>
    route.fulfill({ json: { items: [], total: 0, total_hours: 0 } }),
  )
  await page.route(`**/api/v1/work-packages/${wpB.id}/cost-entries`, (route) =>
    route.fulfill({ json: { items: [], total: 0, total_amount: 0 } }),
  )
  await page.route('**/api/v1/work-packages/*/summary', (route) => {
    const id = route.request().url().includes(wpB.id) ? wpB.id : wpA.id
    return route.fulfill({
      json: {
        work_package_id: id,
        summary: id === wpA.id ? '첫 번째 작업 전용 요약' : '두 번째 작업 전용 요약',
        provider: 'local-extractive',
      },
    })
  })
  await page.setViewportSize({ width: 390, height: 844 })
  await page.goto(`/projects/${project.id}/work-packages/${wpA.id}`)
  let section = page.getByRole('region', { name: 'AI 요약' })
  await section.getByRole('button', { name: '생성', exact: true }).click()
  await expect(section.getByText('첫 번째 작업 전용 요약')).toBeVisible()

  await page.goto(`/projects/${project.id}/work-packages/${wpB.id}`)
  section = page.getByRole('region', { name: 'AI 요약' })
  await expect(page.getByText('수직 슬라이스 데모')).toBeVisible()
  await expect(section.getByText('첫 번째 작업 전용 요약')).toHaveCount(0)
  await expect(section.getByText('아직 생성된 요약이 없습니다.')).toBeVisible()
  await section.getByRole('button', { name: '생성', exact: true }).click()
  await expect(section.getByText('두 번째 작업 전용 요약')).toBeVisible()
  await expect(page.locator('body')).toHaveJSProperty('scrollWidth', 390)
  await page.mouse.move(1, 1)
  await page.screenshot({
    path: '../../docs/screenshots/redevelopment/detail-ai-summary-ui/mobile.png',
    fullPage: true,
  })
})
...
test('문서 목록에서 문서를 열면 편집기가 제목과 본문을 보여준다', async ({ page }) => {
  ...
  await page.route('**/api/v1/document-comments/*', (route) =>
    route.fulfill({ status: 204, body: '' }),
  )
  ...
  await expect(page.getByText('여백 메모')).toBeVisible()
  await expect(page.getByText('코멘트 2건')).toBeVisible()
  const createdComment = page.locator('li').filter({ hasText: '여백 메모' })
  await createdComment.getByRole('button', { name: '코멘트 삭제' }).click()
  await expect(page.getByText('여백 메모')).toHaveCount(0)
  await expect(page.getByText('코멘트 1건')).toBeVisible()
})

test('문서 코멘트는 최신 페이지를 먼저 읽고 이전 페이지 오류를 보존·재시도한다', async ({
  page,
}) => {
  await mockApi(page)
  const doc = {
  ...
  const makeComment = (id: string, body: string, createdAt: string) => ({
    id,
    document_id: 'd1',
    project_id: project.id,
    author_id: 'me-1',
  ...
  await page.route('**/api/v1/documents/d1/comments**', async (route) => {
    ...
    if (!url.searchParams.has('cursor_id')) {
      await route.fulfill({
        json: {
          items: [
            makeComment('new-2', '가장 최신 코멘트', '2026-07-04T00:00:00Z'),
            makeComment('new-1', '두 번째 최신 코멘트', '2026-07-03T00:00:00Z'),
          ],
          total: 4,
          next_cursor_created_at: '2026-07-03T00:00:00Z',
          next_cursor_id: 'new-1',
        },
      })
      return
    }
    olderAttempts += 1
    if (olderAttempts === 1) {
      await route.fulfill({ status: 503, json: { detail: 'temporary pagination failure' } })
      return
    }
    await route.fulfill({
      json: {
        items: [
          makeComment('old-2', '두 번째 이전 코멘트', '2026-07-02T00:00:00Z'),
          makeComment('old-1', '가장 오래된 코멘트', '2026-07-01T00:00:00Z'),
        ],
        total: 4,
        next_cursor_created_at: null,
        next_cursor_id: null,
  ...
  await expect(page.getByText('가장 오래된 코멘트')).toHaveCount(0)
  ...
  await page.getByRole('button', { name: '이전 코멘트 불러오기' }).click()
  await expect(page.getByText('이전 코멘트를 불러오지 못했습니다.')).toBeVisible()
  await expect(page.getByText('가장 최신 코멘트')).toBeVisible()
  await expect(page.getByText('2 / 4개 표시')).toBeVisible()
  await page.getByRole('button', { name: '다시 시도' }).click()
  await expect(page.getByText('가장 오래된 코멘트')).toBeVisible()
  await expect(page.getByText('4 / 4개 표시')).toBeVisible()
  await expect(page.getByRole('button', { name: '이전 코멘트 불러오기' })).toHaveCount(0)
  expect(new URL(requestUrls.at(-1) ?? '').searchParams.get('cursor_id')).toBe('new-1')
  await expectNoHorizontalOverflow(page)
  await page.screenshot({
    path: '../../docs/screenshots/redevelopment/document-comment-pagination-ui/desktop.png',
    fullPage: false,
  })
  ...
  await expectNoHorizontalOverflow(page)
  await page.screenshot({
    path: '../../docs/screenshots/redevelopment/document-comment-pagination-ui/mobile.png',
    fullPage: true,
  })
})
...
test('문서 선택 텍스트에 인라인 코멘트를 만들고 본문과 스레드를 오간다', async ({ page }) => {
  ...
  await expect.poll(() => inlineRequests.length).toBe(2)
...
test('문서 상세 활동 탭은 실제 변경 이력과 재시도·페이지네이션을 연결한다', async ({
  ...
  const activityItems = Array.from({ length: 11 }, (_, index) => ({
    ...
    actor_name: index === 10 ? null : index === 0 ? 'Historical Editor' : 'Dev User',
    actor_profile_image_url:
      index === 0
        ? `/api/v1/documents/${doc.id}/activities/document-activity-0/actor-image?version=11111111-1111-4111-8111-111111111111`
        : null,
    kind:
      index === 0
        ? 'document_restored'
  ...
  await page.route('**/api/v1/documents/activity-doc-1/activities/*/actor-image**', (route) =>
    route.fulfill({
      contentType: 'image/png',
      body: Buffer.from(
        'iVBORw0KGgoAAAANSUhEUgAAAAIAAAACCAIAAAD91JpzAAAAEklEQVR4nGPkndLBwMDAxAAGAA2bAS37E8jFAAAAAElFTkSuQmCC',
  ...
  await expect(page.getByLabel('Historical Editor', { exact: true })).toBeVisible()
  await expect(page.getByLabel('Historical Editor', { exact: true }).locator('img')).toHaveCount(1)
...
test('문서 버전 탭은 내용을 지연 조회·비교하고 현재 버전을 보존하며 복원한다', async ({
  page,
}) => {
  await mockApi(page)
  let doc = {
  ...
  let middleRevisionAttempts = 0
  let restorePayload: { expected_version: number } | null = null
  const currentRevision = {
    id: 'revision-current',
    document_version: 4,
    actor_id: 'me-1',
    actor_name: 'Historical Editor',
    actor_profile_image_url:
      '/api/v1/documents/version-doc-1/revisions/revision-current/actor-image?version=11111111-1111-4111-8111-111111111111',
    title: '운영 가이드',
    body: '<p>현재 운영 절차입니다.</p>',
    changed_fields: ['body'],
  ...
  await page.route('**/api/v1/documents/version-doc-1/comments**', (route) =>
    route.fulfill({ json: { items: [], total: 0 } }),
  )
  await page.route('**/api/v1/documents/version-doc-1/work-package-links', (route) =>
    route.fulfill({ json: { items: [], total: 0 } }),
  )
  await page.route('**/api/v1/documents/version-doc-1/revisions**', async (route) => {
    const request = route.request()
    const path = new URL(request.url()).pathname
    if (request.method() === 'POST' && path.endsWith('/revision-old/restore')) {
      restorePayload = request.postDataJSON() as { expected_version: number }
      restored = true
      doc = {
        ...doc,
    ...
    if (request.method() === 'GET' && path.endsWith('/revision-current')) {
      detailRequests += 1
      await route.fulfill({ json: currentRevision })
      return
    }
    if (request.method() === 'GET' && path.endsWith('/revision-old')) {
      detailRequests += 1
      await route.fulfill({ json: oldRevision })
      return
    }
    if (request.method() === 'GET' && path.endsWith('/revision-middle')) {
      detailRequests += 1
      middleRevisionAttempts += 1
      if (middleRevisionAttempts === 1) {
        await route.fulfill({ status: 500, json: { detail: 'temporary failure' } })
        return
      }
      await route.fulfill({ json: middleRevision })
      return
    }
    const restoredRevision = {
      ...
      actor_name: 'Dev User',
      actor_profile_image_url: null,
      restored_from_revision_id: oldRevision.id,
      created_at: '2026-07-18T05:00:00Z',
    }
    await route.fulfill({
      json: {
        items: restored
          ? [restoredRevision, currentRevision, middleRevision, oldRevision]
          : [currentRevision, middleRevision, oldRevision],
        total: restored ? 4 : 3,
        current_revision_id: restored ? restoredRevision.id : currentRevision.id,
      },
    })
  })
  await page.route('**/api/v1/documents/version-doc-1/revisions/*/actor-image**', (route) =>
    route.fulfill({
      contentType: 'image/png',
      body: Buffer.from(
        'iVBORw0KGgoAAAANSUhEUgAAAAIAAAACCAIAAAD91JpzAAAAEklEQVR4nGPkndLBwMDAxAAGAA2bAS37E8jFAAAAAElFTkSuQmCC',
  ...
  await page.route('**/api/v1/documents/version-doc-1', (route) =>
    route.fulfill({ json: doc }),
  )
  ...
  await expect(page.getByRole('button', { name: /버전 4.*현재/ })).toBeVisible()
  await expect(page.getByLabel('Historical Editor', { exact: true })).toHaveCount(2)
  await expect(page.getByLabel('Historical Editor', { exact: true }).first().locator('img')).toHaveCount(1)
  await expect(page.getByLabel('문서 버전 4 본문 미리보기')).toContainText('현재 운영 절차입니다.')
  expect(detailRequests).toBe(1)

  await page.getByRole('button', { name: /버전 2/ }).click()
  await expect(page.getByLabel('문서 버전 2 본문 미리보기')).toContainText(
    '과거 운영 절차와 체크리스트입니다.',
  )
  const formerMemberLabels = page.getByText('이전 구성원', { exact: true })
  await expect(formerMemberLabels).toHaveCount(2)
  await expect(formerMemberLabels.first()).toBeVisible()
  await expect(formerMemberLabels.last()).toBeVisible()
  expect(detailRequests).toBe(2)

  await page.getByRole('button', { name: '버전 비교' }).click()
  await expect(page.getByRole('heading', { name: '버전 비교' })).toBeVisible()
  await expect(page.getByLabel('기준 버전', { exact: true })).toHaveValue('revision-old')
  await expect(page.getByLabel('비교 버전', { exact: true })).toHaveValue('revision-current')
  await expect(page.getByLabel('본문 변경 비교').locator('del').first()).toContainText('과거')
  await expect(page.getByLabel('본문 변경 비교').locator('ins').first()).toContainText('현재')

  await page.getByLabel('기준 버전', { exact: true }).selectOption('revision-middle')
  await expect(page.getByRole('alert').getByText('비교할 버전 내용을 불러오지 못했습니다.')).toBeVisible()
  await page.getByRole('alert').getByRole('button', { name: '재시도' }).click()
  await expect(page.getByLabel('제목 변경 비교').locator('del')).toContainText('승인')

  await page.getByRole('heading', { name: '버전 비교' }).evaluate((element) => {
    element.scrollIntoView({ block: 'start' })
  })
  await page.screenshot({
    path: '../../docs/screenshots/redevelopment/document-version-compare-ui/inline-desktop.png',
    fullPage: true,
  })

  await page.getByRole('radio', { name: '나란히' }).click()
  await expect(page.getByLabel('기준 버전 3 본문')).toContainText('승인 단계를 추가했습니다.')
  await expect(page.getByLabel('비교 버전 4 본문')).toContainText('현재 운영 절차입니다.')
  await page.getByRole('button', { name: '기준과 비교 버전 바꾸기' }).click()
  await expect(page.getByLabel('기준 버전', { exact: true })).toHaveValue('revision-current')
  await expect(page.getByLabel('비교 버전', { exact: true })).toHaveValue('revision-middle')

  await page.getByRole('heading', { name: '버전 비교' }).evaluate((element) => {
    element.scrollIntoView({ block: 'start' })
  })
  await page.screenshot({
    path: '../../docs/screenshots/redevelopment/document-version-compare-ui/desktop.png',
    fullPage: true,
  })
  await page.setViewportSize({ width: 390, height: 844 })
  await page.getByRole('heading', { name: '버전 비교' }).evaluate((element) => {
  ...
  await expectNoHorizontalOverflow(page)
  await page.screenshot({
    path: '../../docs/screenshots/redevelopment/document-version-compare-ui/mobile.png',
    fullPage: true,
  })
  await page.setViewportSize({ width: 1440, height: 900 })
  await page.getByRole('button', { name: '버전 비교 닫기' }).click()

  page.once('dialog', (dialog) => dialog.accept())
  const restoreResponse = page.waitForResponse(
    (response) =>
      response.request().method() === 'POST' && response.url().endsWith('/revision-old/restore'),
  )
  ...
  await page.screenshot({
    path: '../../docs/screenshots/redevelopment/document-version-history-ui/desktop.png',
    fullPage: true,
  })
  await page.setViewportSize({ width: 390, height: 844 })
  await page.getByRole('tablist', { name: '문서 상세 보기' }).evaluate((element) => {
  ...
  await expectNoHorizontalOverflow(page)
  await page.screenshot({
    path: '../../docs/screenshots/redevelopment/document-version-history-ui/mobile.png',
    fullPage: true,
  })
})
...
test('문서 편집기에서 작업을 연결하고 드로어 페이지 섹션이 링크를 보여준다', async ({
  ...
  await page.goto(`/projects/${project.id}/work-packages`)
  await page.getByRole('button', { name: wpA.subject, exact: true }).click()
...
test('목록 로딩 스켈레톤은 모바일에서 콘텐츠 폭을 넘지 않는다', async ({ page }) => {
  ...
  await page.goto(`/projects/${project.id}/work-packages`)
  const listSkeleton = page.getByRole('status', { name: '불러오는 중', exact: true })
  await expect(listSkeleton).toBeVisible()
  await expectNoHorizontalOverflow(page)
  await listSkeleton.screenshot({
...
test('목록이 여러 페이지를 모두 불러온다 (200건 무성 절단 없음)', async ({ page }) => {
  ...
  await expect(page.getByTestId('work-item-total')).toHaveText('501개')
})
...
test('마일스톤 패널이 행 작업 메뉴·편집·삭제 확인·필터 이동을 제공한다', async ({ page }) => {
  ...
  const openMilestoneWork = page.getByLabel('1차 출시 작업 목록 열기')
  await milestoneMenuTrigger.focus()
  await milestoneMenuTrigger.press('Enter')
  await expect(openMilestoneWork).toBeFocused()
  await page.keyboard.press('ArrowDown')
  await expect(page.getByLabel('1차 출시 편집')).toBeFocused()
  await page.keyboard.press('End')
  await expect(milestoneMenu.getByRole('menuitem', { name: '닫기' })).toBeFocused()
  await page.keyboard.press('Escape')
  await expect(milestoneMenu).toHaveCount(0)
  await expect(milestoneMenuTrigger).toBeFocused()

  await milestoneMenuTrigger.click()
  await expect(openMilestoneWork).toBeFocused()
  await page.getByText('마일스톤').first().click()
  await expect(milestoneMenu).toHaveCount(0)
...
test('마일스톤 행 작업 메뉴가 모바일 폭 안에 머물고 읽기 전용 cue를 보여준다', async ({ page }) => {
  ...
  await expect(menu).toBeVisible()
  await expect(menu.getByText('쓰기 권한 없음')).toBeVisible()
  await expect(page.getByLabel('모바일 출시 작업 목록 열기')).toBeFocused()
  await page.keyboard.press('ArrowDown')
  await expect(menu.getByRole('menuitem', { name: '닫기' })).toBeFocused()
...
test('로그인 화면은 참조 시안의 기능 계약을 유지하고 안전하게 이동한다', async ({ page }) => {
  ...
  await expect(showPasswordButton.locator('.of-login-password-hidden-icon')).toBeVisible()
...
test('로그인 참조 UI는 10개 목표 뷰포트에서 넘침과 compact 충돌 없이 렌더링된다', async ({ page }) => {
  test.setTimeout(90_000)
  await page.emulateMedia({ reducedMotion: 'reduce' })
  await mockApi(page)
  await page.route('**/api/v1/auth/config', (route) => route.fulfill({
  ...
  const viewports = [
    ...
    { name: 'in-app-914x800', width: 914, height: 800 },
    { name: '768x1024', width: 768, height: 1024 },
    { name: 'in-app-562x734', width: 562, height: 734 },
  ...
  for (const viewport of viewports) {
    ...
    if (viewport.width >= 521) {
      ...
      const geometry = await page.evaluate(() => {
        ...
        const cardElement = document.querySelector('.of-login-auth-card') as HTMLElement
        return {
          splitGap: Math.abs(story.right - auth.left),
          cardScrollbarWidth: getComputedStyle(cardElement).scrollbarWidth,
          cardContentWidthPreserved:
            cardElement.clientWidth === cardElement.scrollWidth,
          cardInsideAuth:
      ...
      expect(geometry.cardScrollbarWidth).toBe('none')
      expect(geometry.cardContentWidthPreserved).toBe(true)
    ...
    if (viewport.name === '1448x1086') {
      await page.screenshot({
        path: '../../docs/screenshots/redevelopment/login-pixel-reinspection-ui/reference-size.png',
        fullPage: true,
      })
    }
  }
...
test('인앱 compact desktop은 승인 원본의 전체 4:3 프레임과 기능형 폼을 함께 유지한다', async ({ page }) => {
  await mockApi(page)
  await page.route('**/api/v1/auth/config', (route) => route.fulfill({
    json: {
      auth_mode: 'dev',
  ...
  await page.setViewportSize({ width: 562, height: 734 })
  await page.goto('/login')
  await page.locator('.of-login-story-art, .of-login-brand-reference img').evaluateAll(
  ...
  const geometry = await page.evaluate(() => {
    ...
    const panel = document.querySelector('.of-login-page') as HTMLElement
    const heading = document.querySelector('.of-login-heading h2') as HTMLElement
    return {
      panel: box('.of-login-page'),
      story: box('.of-login-story'),
      auth: box('.of-login-auth'),
      card: box('.of-login-auth-card'),
      footer: box('.of-login-footer'),
      zoom: Number(getComputedStyle(panel).zoom),
  ...
  expect(geometry.panel).toEqual({ x: 0, y: 156.25, width: 562, height: 421.5 })
  expect(geometry.story.width / geometry.panel.width).toBeCloseTo(792 / 1448, 4)
  expect(Math.abs(geometry.story.x + geometry.story.width - geometry.auth.x)).toBeLessThan(0.05)
  expect(geometry.card.x).toBeGreaterThanOrEqual(geometry.auth.x)
  expect(geometry.card.x + geometry.card.width).toBeLessThanOrEqual(geometry.auth.x + geometry.auth.width)
  expect(geometry.footer.y + geometry.footer.height).toBeLessThanOrEqual(
    geometry.panel.y + geometry.panel.height,
  )
  const expectedScale = 562 / 1448
  expect(geometry.zoom).toBe(1)
  expect(geometry.transform).toBe('none')
  expect(geometry.headingFontSize).toBeCloseTo(32.5 * expectedScale, 1)
  ...
  await expect(page.locator('.of-login-story-art')).toHaveAttribute(
    'srcset',
    /oneflow-login-story-reference@2x/,
  )
  await expect(page.locator('.of-login-brand-reference img')).toHaveAttribute(
    'srcset',
    /oneflow-login-logo-lockup@2x/,
  )
  await page.screenshot({
    path: '../../docs/screenshots/redevelopment/login-direct-rendering-ui/in-app-562x734.png',
  })
  ...
  await expect(page.getByRole('heading', { name: /Welcome back/ })).toBeVisible()
  await page.screenshot({
    path: '../../docs/screenshots/redevelopment/login-origin-exhaustive-ui/desktop-1448x1086.png',
  })
})

test('인앱 914x800 로그인은 승인 원본 기하를 최종 크기로 직접 렌더링한다', async ({ page }) => {
  await mockApi(page)
  await page.route('**/api/v1/auth/config', (route) => route.fulfill({
    json: {
      auth_mode: 'dev',
  ...
  const geometry = await page.evaluate(() => {
    ...
    const panel = document.querySelector('.of-login-page') as HTMLElement
    const heading = document.querySelector('.of-login-heading h2') as HTMLElement
    const email = document.querySelector('#login-email') as HTMLElement
    return {
      panel: read('.of-login-page'),
  ...
  const expectedScale = 914 / 1448
  expect(geometry.zoom).toBe(1)
  expect(geometry.transform).toBe('none')
  expect(geometry.panel).toMatchObject({ x: 0, width: 914 })
  expect(geometry.panel.y).toBeCloseTo((800 - (1086 * expectedScale)) / 2, 1)
  expect(geometry.panel.height).toBeCloseTo(1086 * expectedScale, 1)
  expect(geometry.story.width / geometry.panel.width).toBeCloseTo(792 / 1448, 5)
  expect(Math.abs(geometry.story.x + geometry.story.width - geometry.auth.x)).toBeLessThan(0.05)
  expect(geometry.story.currentSrc).toMatch(/oneflow-login-story-reference-525x720\.png/)
  expect(geometry.story.naturalWidth).toBeGreaterThan(0)
  expect(geometry.story.naturalHeight).toBeGreaterThan(0)
  expect(geometry.logo.currentSrc).toMatch(/oneflow-login-logo-lockup\.png/)
  expect(geometry.logo).toMatchObject({ naturalWidth: 205, naturalHeight: 70 })
  expect(geometry.card.x).toBeCloseTo((792 + 60) * expectedScale, 1)
  expect(geometry.card.y).toBeCloseTo(geometry.panel.y + (70 * expectedScale), 1)
  expect(geometry.headingFontSize).toBeCloseTo(32.5 * expectedScale, 1)
  expect(geometry.emailFontSize).toBeCloseTo(14.5 * expectedScale, 1)
  expect(geometry.viewport).toEqual({
    width: 914,
    height: 800,
    scrollWidth: 914,
  ...
  await expectNoHorizontalOverflow(page)
  await page.screenshot({
    path: '../../docs/screenshots/redevelopment/login-direct-rendering-ui/in-app-914x800.png',
  })
  ...
  await expectNoHorizontalOverflow(page)
  await page.screenshot({
    path: '../../docs/screenshots/redevelopment/login-direct-rendering-ui/desktop-1448x1086.png',
  })
  ...
  await expectNoHorizontalOverflow(page)
  await page.screenshot({
    path: '../../docs/screenshots/redevelopment/login-direct-rendering-ui/mobile-390x844.png',
    fullPage: true,
  })
})
...
test('로그인 원본 비주얼과 흐르는 협업 경로 강조를 유지한다', async ({ page }) => {
  ...
  await expect(storyArt).toHaveAttribute('srcset', /oneflow-login-story-reference@2x/)
  await storyArt.evaluate(async (image) => {
    await Promise.all(image.getAnimations().map((animation) => animation.finished))
  })
  const storyImage = await storyArt.evaluate((image) => {
    const typedImage = image as HTMLImageElement
    const rect = typedImage.getBoundingClientRect()
    return {
      currentSrc: typedImage.currentSrc,
      naturalWidth: typedImage.naturalWidth,
      naturalHeight: typedImage.naturalHeight,
      renderedWidth: rect.width,
      renderedHeight: rect.height,
    }
  })
  expect(storyImage.currentSrc).toMatch(/oneflow-login-story-reference-667x915\.png/)
  expect(storyImage.naturalWidth).toBeGreaterThan(0)
  expect(storyImage.naturalHeight).toBeGreaterThan(0)
  expect(storyImage.renderedWidth / storyImage.renderedHeight).toBeCloseTo(792 / 1086, 4)
  const sourceSplit = await page.evaluate(() => {
    const panel = document.querySelector('.of-login-page')?.getBoundingClientRect()
    const story = document.querySelector('.of-login-story')?.getBoundingClientRect()
    if (!panel || !story) throw new Error('Missing login reference geometry')
    return story.width / panel.width
  })
  expect(Math.abs(sourceSplit - (792 / 1448))).toBeLessThan(0.0001)
  ...
  await expect(routeFlow).toHaveCSS('stroke-dasharray', '2px, 20px')
  await expect(routeFlow).toHaveCSS('stroke-width', '2.25px')
  await expect(routeFlow).toHaveCSS('stroke', 'rgba(255, 255, 255, 0.42)')
  const headingTransform = await page.locator('.of-login-heading h2').evaluate((element) => {
    const matrix = new DOMMatrix(getComputedStyle(element).transform)
    const emoji = element.querySelector('span') as HTMLElement
    const emojiMatrix = new DOMMatrix(getComputedStyle(emoji).transform)
    const panel = document.querySelector('.of-login-page')!.getBoundingClientRect()
    return {
      scaleX: matrix.a,
      x: matrix.e,
      y: matrix.f,
      emojiX: emojiMatrix.e,
      emojiY: emojiMatrix.f,
      panelWidth: panel.width,
    }
  })
  const headingScale = headingTransform.panelWidth / 1448
  expect(headingTransform.scaleX).toBeCloseTo(0.96, 4)
  expect(headingTransform.x).toBeCloseTo(-2.25 * headingScale, 4)
  expect(headingTransform.y).toBeCloseTo(-0.75 * headingScale, 4)
  expect(headingTransform.emojiX).toBeCloseTo(1.5 * headingScale, 4)
  expect(headingTransform.emojiY).toBeCloseTo(-4.5 * headingScale, 4)
  ...
  await page.reload()
  await expect(page.getByRole('heading', { name: /Welcome back/ })).toBeVisible()
  await page.locator('.of-login-story-art, .of-login-auth-card').evaluateAll(
    async (elements) => {
...
test('로그인 최대 데스크톱 합성은 원본 파생 자산을 정수 픽셀 격자에 배치한다', async ({
  page,
}) => {
  await mockApi(page)
  await page.route('**/api/v1/auth/config', (route) => route.fulfill({
  ...
  await page.setViewportSize({ width: 1456, height: 1260 })
  await page.goto('/login')
  await page.locator('.of-login-story-art, .of-login-brand-reference img').evaluateAll(
    async (images) => {
      await Promise.all(images.map((image) => (image as HTMLImageElement).decode()))
  ...
  await expectNoHorizontalOverflow(page)
  await page.screenshot({
    path: '../../docs/screenshots/redevelopment/login-pixel-reinspection-ui/desktop.png',
    fullPage: true,
  })
...
test('로그인 reference-fit은 인앱 크기와 기준 크기에서 정수 raster를 선택한다', async ({ page }) => {
  ...
  expect(inAppGeometry.story.currentSrc).toMatch(/oneflow-login-story-reference-525x720\.png/)
  ...
  expect(referenceGeometry.story.naturalWidth).toBeGreaterThanOrEqual(791)
  expect(referenceGeometry.story.naturalWidth).toBeLessThanOrEqual(792)
  expect(referenceGeometry.story.naturalHeight).toBeGreaterThanOrEqual(1085)
  expect(referenceGeometry.story.naturalHeight).toBeLessThanOrEqual(1086)
  ...
  await expect(page.locator('.of-login-auth-card')).toHaveCSS('animation-name', 'none')
  await expect(page.locator('.of-login-heading h2')).toHaveCSS('font-weight', '600')
...
test('로그인 인증 패널은 승인 원본의 픽셀 보정과 라이브 비밀번호 제어를 유지한다', async ({ page }) => {
  await page.emulateMedia({ reducedMotion: 'reduce' })
  await mockApi(page)
  await page.route('**/api/v1/auth/config', (route) => route.fulfill({
    json: {
  ...
  await expect(page.locator('.of-login-password-hidden-icon')).toHaveClass(/lucide-eye-off/)
  await expect(page.locator('.of-login-provider-button').first()).toHaveCSS('font-weight', '500')
  await expectNoHorizontalOverflow(page)
  await page.screenshot({
    path: '../../docs/screenshots/redevelopment/login-auth-pixel-parity-ui/desktop-1448x1086.png',
  })
  ...
  await expect(page.locator('.of-login-password-toggle .lucide-eye')).toBeVisible()
  await page.getByRole('button', { name: 'Hide password' }).click()
  await expect(password).toHaveAttribute('type', 'password')
  await expect(page.locator('.of-login-password-hidden-icon')).toBeVisible()
  ...
  await expectNoHorizontalOverflow(page)
  await page.screenshot({
    path: '../../docs/screenshots/redevelopment/login-auth-pixel-parity-ui/in-app-914x800.png',
  })
  ...
  await expectNoHorizontalOverflow(page)
  await page.screenshot({
    path: '../../docs/screenshots/redevelopment/login-auth-pixel-parity-ui/mobile-390x844.png',
    fullPage: true,
  })
})

test('로그인 승인 원본 비주얼은 처음부터 기능형 인증 화면과 동일한 상태로 유지된다', async ({ page }) => {
  test.setTimeout(60_000)
  await mockApi(page)
  await page.route('**/api/v1/auth/config', (route) => route.fulfill({
    json: {
  ...
  await expect(pageFrame).not.toHaveAttribute('data-reference-state', /.+/)
  await expect(page.locator('.of-login-reference-stage')).toHaveCount(0)
  await expect(page.locator('.of-login-story-art')).toHaveAttribute(
    'srcset',
    /oneflow-login-story-reference@2x/,
  )
...
test('로그인 브랜드는 DPR 2에서 승인 원본 파생 2x lockup을 실제 선택한다', async ({
  baseURL,
  browser,
}) => {
  const context = await browser.newContext({
    ...
    deviceScaleFactor: 2,
    viewport: { width: 1455, height: 1259 },
  })
  ...
  try {
    ...
    await page.goto('/login')
    const brand = page.locator('.of-login-brand-reference img')
    await expect(brand).toBeVisible()
    expect(await brand.evaluate((image) => (image as HTMLImageElement).currentSrc))
      .toMatch(/oneflow-login-logo-lockup@2x/)
    await expectNoHorizontalOverflow(page)
  } finally {
    await context.close()
  }
...
test('사용자 디렉터리는 서버 검색·필터·추가 페이지와 전체 요약을 유지한다', async ({ page }) => {
  ...
  await page.route(/\/api\/v1\/users(?:\?.*)?$/, (route) => {
    ...
    const url = new URL(route.request().url())
    requests.push(url)
    if (url.searchParams.get('scope') === 'inactive') {
      return route.fulfill({ json: { items: [inactive], total: 1, summary } })
    }
    if (url.searchParams.get('q')?.toLowerCase() === 'alex') {
      return route.fulfill({ json: { items: [alex], total: 1, summary } })
    }
    if (url.searchParams.get('offset') === '2') {
      return route.fulfill({ json: { items: [inactive], total: 3, summary } })
    }
    return route.fulfill({ json: { items: [alex, admin], total: 3, summary } })
  })

  await page.goto('/admin/users')
  await expect(page.getByText('2 / 3명 표시')).toBeVisible()
  const adminRow = page.getByRole('row', { name: /Dev User/ })
  await expect(adminRow.getByRole('button', { name: '비활성화' })).toBeDisabled()
  await expect(adminRow.getByLabel('Dev User 관리자 권한')).toBeDisabled()

  await page.getByRole('button', { name: '더 불러오기' }).click()
  await expect(page.getByText('3 / 3명 표시')).toBeVisible()
  ...
  await expect(page.getByText('1 / 1명 표시')).toBeVisible()
  await expect(page.getByText('old@oneflow.local')).toBeVisible()
  await expect(page.getByText('alex@oneflow.local')).toBeHidden()
  expect(requests.some((url) => url.searchParams.get('scope') === 'inactive')).toBe(true)

  await page.getByRole('button', { name: '전체', exact: true }).click()
  await expect(page).not.toHaveURL(/scope=/)
  await page.getByLabel('사용자 검색').fill('alex')
  await expect(page).toHaveURL(/q=alex/)
  await expect(page).not.toHaveURL(/scope=/)
  await expect(page.getByText('1 / 1명 표시')).toBeVisible()
  await expect(page.getByText('alex@oneflow.local')).toBeVisible()
  await expect(page.getByText('dev@oneflow.local')).toBeHidden()
  expect(requests.some((url) => url.searchParams.get('q') === 'alex')).toBe(true)
})
...
test('사용자 디렉터리는 모바일에서 계정 카드와 멤버십을 유지한다', async ({ page }) => {
  ...
  await page.route(/\/api\/v1\/users(?:\?.*)?$/, (route) => {
    const url = new URL(route.request().url())
    const items = url.searchParams.get('scope') === 'admins' ? [admin] : [admin, alex, oldUser]
    return route.fulfill({ json: { items, total: items.length, summary } })
  })
...
test('프로젝트 목록 정렬이 순서를 바꾸고 방향 토글이 동작한다', async ({ page }) => {
  ...
  await page.route(/\/api\/v1\/projects(?:\?.*)?$/, (route) => {
    ...
    const descending = url.searchParams.get('sort_direction') === 'desc'
    const sorted = url.searchParams.get('sort_key') === 'overdue_count'
      ? [...items].sort((left, right) =>
          descending
            ? right.overdue_count - left.overdue_count
            : left.overdue_count - right.overdue_count,
        )
      : items
    return route.fulfill({ json: { items: sorted, total: 2 } satisfies ProjectList })
  })
  await page.goto('/projects')
  ...
  await expect.poll(() => requests.at(-1)).toContain('sort_key=overdue_count')
  await expect(rows.first()).toContainText('알파')
  await page.getByLabel(/정렬 방향/).click() // asc → desc
  await expect.poll(() => requests.at(-1)).toContain('sort_direction=desc')
  await expect(rows.first()).toContainText('베타') // 5 overdue first
  const descendingButton = page.getByRole('button', { name: '정렬 방향 (내림차순)' })
  await expect(descendingButton).toBeVisible()
  await descendingButton.click() // back to asc
  await expect(page.getByRole('button', { name: '정렬 방향 (오름차순)' })).toBeVisible()
  await expect(rows.first()).toContainText('알파')
})
...
test('프로젝트 디렉터리는 서버 페이지·검색 URL·보관 범위와 전체 템플릿 옵션을 보존한다', async ({
  ...
  const directoryItems = [
    projectListItem('page-1', '기획 허브', { key: 'PLAN' }),
    projectListItem('page-2', '개발 허브', { key: 'DEV' }),
    projectListItem('page-3', '운영 자동화', { key: 'OPS', overdue_count: 3 }),
    projectListItem('page-4', '고객 포털', { key: 'PORTAL' }),
    projectListItem('page-5', '운영 보관함', {
      key: 'ARC',
      archived_at: '2026-07-21T00:00:00Z',
    }),
  ]
  let failDirectoryPageOnce = true
  await page.route(/\/api\/v1\/projects(?:\?.*)?$/, (route) => {
    const url = new URL(route.request().url())
    const includeArchived = url.searchParams.get('include_archived') === 'true'
    const query = url.searchParams.get('q')?.trim() ?? ''
    ...
    if (requestedLimit === 200 && offset === 2 && failDirectoryPageOnce) {
      failDirectoryPageOnce = false
      return route.fulfill({ status: 503, json: { detail: 'temporarily unavailable' } })
    }
    const scope = directoryItems.filter((item) => includeArchived || !item.archived_at)
    const filtered = query ? scope.filter((item) => item.name.includes(query)) : scope
    const summary = {
      projects: scope.length,
      active: scope.filter((item) => !item.archived_at).length,
      archived: scope.filter((item) => item.archived_at).length,
      open_work_packages: scope.reduce((sum, item) => sum + item.open_work_package_count, 0),
      overdue_work_packages: scope.reduce((sum, item) => sum + item.overdue_count, 0),
      initiatives: 0,
    }
    const limit = requestedLimit === 500 ? 500 : 2
    return route.fulfill({
      json: {
        items: filtered.slice(offset, offset + limit),
        total: filtered.length,
  ...
  await expect(page.getByText('2 / 4개 표시')).toBeVisible()
  await page.screenshot({
    path: '../../docs/screenshots/redevelopment/project-directory-pagination-ui/desktop.png',
    fullPage: false,
  })

  await page.getByRole('button', { name: '더 불러오기' }).click()
  await expect(page.getByRole('alert')).toContainText('추가 프로젝트를 불러오지 못했습니다')
  await expect(directory.getByRole('listitem')).toHaveCount(2)
  await page.getByRole('button', { name: '더 불러오기' }).click()
  await expect(directory.getByRole('listitem')).toHaveCount(4)
  await expect(page.getByRole('button', { name: '더 불러오기' })).toHaveCount(0)

  await page.getByLabel('프로젝트 검색어').fill('운영')
  await expect(page).toHaveURL(/q=%EC%9A%B4%EC%98%81/)
  await expect(directory).toContainText('운영 자동화')
  await expect(directory).not.toContainText('개발 허브')
  await page.reload()
  await expect(page.getByRole('textbox', { name: '프로젝트 검색어' })).toHaveValue('운영')
  await expect(directory).toContainText('운영 자동화')
  ...
  await expect(includeArchived).toBeChecked()
  await expect(directory).toContainText('운영 보관함')
  await expect(page.getByLabel('프로젝트 요약')).toContainText('보관 1')

  await page.getByRole('button', { name: /새 프로젝트/ }).click()
  await expect(page.getByLabel('템플릿으로 사용할 프로젝트')).toContainText('고객 포털')

  await page.setViewportSize({ width: 390, height: 844 })
  await expectNoHorizontalOverflow(page)
  await page.screenshot({
    path: '../../docs/screenshots/redevelopment/project-directory-pagination-ui/mobile.png',
    fullPage: true,
  })
})
...
test('프로젝트 cover는 디렉터리와 Overview를 공유하고 owner가 교체·제거한다', async ({ page }) => {
  ...
  await expect(page.locator('[data-slot="modal-overlay"]')).toHaveAttribute('data-state', 'open')
  await dialog.getByRole('button', { name: '표지 창 닫기' }).click()
  await expect(dialog).toHaveAttribute('data-state', 'closed')
  await expect(dialog).toHaveCount(0)
  await expect(coverTrigger).toBeFocused()
  await coverTrigger.click()
  const coverViewportCenter = await page.evaluate(() => window.innerWidth / 2)
  await expect.poll(async () => {
    const box = await dialog.boundingBox()
    return box ? Math.round(box.x + box.width / 2) : null
  }).toBe(Math.round(coverViewportCenter))
  await page.waitForTimeout(250)
  await page.screenshot({
    path: '../../docs/screenshots/redevelopment/project-modal-motion-ui/cover-desktop.png',
  })
...
test('프로젝트 일정 기준선 이력과 추세는 이름 저장·시간순 선택·개별 삭제까지 실제 요청으로 이어진다', async ({ page }) => {
  ...
  await page.route(`**/api/v1/projects/${project.id}/schedule-baselines**`, async (route) => {
    ...
    if (!suffix && request.method() === 'POST') {
      ...
      history.items.unshift({
        ...baseline,
        total_snapshot: created.total_snapshot,
        comparison_total: created.total_snapshot + created.added,
        changed_total: created.changed_total,
        risk_total: created.later + created.unscheduled + created.removed,
      })
  ...
  await createDialog.getByRole('button', { name: '기준선 저장 창 닫기' }).click()
  await expect(createDialog).toHaveAttribute('data-state', 'closed')
  await expect(createDialog).toHaveCount(0)
  await expect(panel.getByRole('button', { name: '첫 기준선 저장' })).toBeFocused()
  await panel.getByRole('button', { name: '첫 기준선 저장' }).click()
  await page.waitForTimeout(250)
  await page.screenshot({
    path: '../../docs/screenshots/redevelopment/project-modal-motion-ui/baseline-create-desktop.png',
  })
  ...
  Object.assign(history.items.find((entry) => entry.id === unchanged.baseline!.id)!, {
    comparison_total: changed.total_snapshot + changed.added,
    changed_total: changed.changed_total,
    risk_total: changed.later + changed.unscheduled + changed.removed,
  })
  await page.reload()
  ...
  const trend = panel.getByRole('region', { name: '기준선 일정 추세' })
  await expect(trend.getByRole('button')).toHaveCount(2)
  await expect(trend.getByRole('button', { name: /착수 승인 기준선 선택, 변동 4개, 위험 2개/ })).toBeVisible()
  await trend.getByRole('button', { name: /착수 승인 기준선 선택/ }).click()
  await expect(panel.getByLabel('비교할 기준선')).toHaveValue(unchanged.baseline!.id)
  ...
  await page.screenshot({
    path: '../../docs/screenshots/redevelopment/project-schedule-baseline-trend-ui/desktop.png',
    fullPage: true,
  })

  await page.setViewportSize({ width: 390, height: 844 })
  const mobilePanel = page.getByRole('region', { name: '프로젝트 일정 기준선' })
  await expect(mobilePanel.getByRole('region', { name: '기준선 일정 추세' }).getByRole('button')).toHaveCount(2)
  await expectNoHorizontalOverflow(page)
  await mobilePanel.scrollIntoViewIfNeeded()
  await page.screenshot({
    path: '../../docs/screenshots/redevelopment/project-schedule-baseline-trend-ui/mobile.png',
    fullPage: true,
  })
  ...
  const deleteViewportCenter = await page.evaluate(() => window.innerWidth / 2)
  await expect.poll(async () => {
    const box = await dialog.boundingBox()
    return box ? Math.round(box.x + box.width / 2) : null
  }).toBe(Math.round(deleteViewportCenter))
  await expectNoHorizontalOverflow(page)
  await page.waitForTimeout(250)
  await page.screenshot({
    path: '../../docs/screenshots/redevelopment/project-modal-motion-ui/baseline-delete-mobile.png',
  })
...
test('프로젝트 Overview 상태 보고 이력은 전환과 작성자를 최신순으로 표시한다', async ({ page }) => {
  ...
  const actorImage = `/api/v1/projects/${project.id}/health-history/history-3/actor-image?version=11111111-1111-4111-8111-111111111111`
  let actorImageRequests = 0
  const dashboard = {
    id: project.id,
    key: project.key,
    ...
    items: [
      {
        ...
        changed_by_profile_image_url: actorImage,
      ...
      },
      {
        ...
        changed_by_profile_image_url: null,
  ...
  await page.route('**/api/v1/projects/*/health-history/*/actor-image**', (route) => {
    actorImageRequests += 1
    return route.fulfill({ body: avatarPng, contentType: 'image/png' })
  })
  await page.route('**/api/v1/projects/*/health-history**', async (route) => {
    ...
    return route.fulfill({ json: healthHistory })
  })
...
test('프로젝트 디렉터리는 모바일에서 요약·검색·카드 링크가 겹치지 않는다', async ({ page }) => {
  ...
  await page.route(/\/api\/v1\/projects(?:\?.*)?$/, (route) => {
    ...
    const items = query
      ? mobileProjects.filter((item) => item.name.includes(query))
      : mobileProjects
    return route.fulfill({ json: { items, total: items.length } satisfies ProjectList })
  })
...
test('관리자 direct route는 identity pending 동안 shell geometry를 비상호작용 상태로 보존한다', async ({ page }) => {
  await mockApi(page)
...
test('모바일 관리자 shell은 identity pending navigation을 drawer 안에서 원자적으로 교체한다', async ({ page }) => {
  await page.setViewportSize({ width: 390, height: 844 })
  await mockApi(page)
  let releaseIdentity: (() => void) | undefined
  const identityGate = new Promise<void>((resolve) => {
  ...
  const mobileNav = page.getByRole('dialog', { name: '모바일 내비게이션' })
    .getByRole('navigation', { name: '설정 컨텍스트 내비게이션' })
  await expect(mobileNav.getByRole('status', { name: '관리자 내비게이션 불러오는 중' })).toBeVisible()
  await expect(mobileNav.getByRole('link', { name: '연결 및 통합' })).toHaveCount(0)
  await expectNoHorizontalOverflow(page)

  releaseIdentity?.()
  await expect(mobileNav.getByRole('link', { name: '연결 및 통합' })).toHaveAttribute('aria-current', 'page')
  await expect(mobileNav.getByRole('status', { name: '관리자 내비게이션 불러오는 중' })).toHaveCount(0)
  await expectNoHorizontalOverflow(page)
})

test('identity 오류 shell은 비인가 링크 없이 재시도해 관리자 navigation을 복구한다', async ({ page }) => {
  await mockApi(page)
  let identityReads = 0
  await page.route('**/api/v1/me', async (route) => {
    identityReads += 1
    if (identityReads === 1) {
      await route.fulfill({ status: 403, json: { detail: 'Identity unavailable' } })
      return
    }
    await route.fulfill({
      json: {
...
test('모바일 백로그 항목 액션 메뉴는 cycle select와 충돌하지 않고 폭을 넘지 않는다', async ({ page }) => {
  ...
  const action = page.getByRole('button', { name: '워크패키지 API 구현 백로그 항목 작업' })
  await expect(action).toHaveAttribute('aria-expanded', 'true')
  await expect(action).toHaveAttribute('aria-controls', `backlog-actions-${wpA.id}`)
  ...
  await action.click()
  await expect(menu).toBeVisible()
  await page.getByRole('heading', { name: '백로그' }).click()
  await expect(menu).toBeHidden()
...
test('뷰어 백로그 항목 액션 메뉴는 쓰기 액션 없이 읽기 전용으로 표시된다', async ({ page }) => {
  ...
  await expect(menu).toBeHidden()
  await expect(
    page.getByRole('button', { name: '워크패키지 API 구현 백로그 항목 작업' }),
  ).toBeFocused()
})
...
test('포트폴리오 리포트가 행·합계·아카이브 토글을 보여준다', async ({ page }) => {
  ...
  const active = {
    ...
    cost_total: 5000000,
    hours_total: 42.5,
    schedule_baseline_id: 'baseline-1',
    schedule_baseline_name: '출시 기준선',
  ...
  await expect(page.getByRole('table').getByText('주의', { exact: true })).toBeVisible() // health chip
  ...
  const archivedRow = page.getByRole('row').filter({ hasText: '종료 프로젝트' })
  await expect(archivedRow.getByRole('cell', { name: '미설정' })).toHaveCount(2) // baseline + NULL budget
  await page.screenshot({
    path: '../../docs/screenshots/redevelopment/portfolio-schedule-baseline-ui/desktop.png',
    fullPage: true,
  })
  const scheduleFilter = page.getByRole('group', { name: '일정 기준선 필터' })
  await scheduleFilter.getByRole('button', { name: '미설정 1' }).click()
  await expect(page.getByRole('button', { name: '종료 프로젝트', exact: true })).toBeVisible()
...
test('Workspace 관리 개요는 실제 운영 상태와 기본 관리 동선을 독립적으로 제공한다', async ({ page }) => {
  await mockApi(page)
  let profileReads = 0
  ...
  await page.route('**/api/v1/users', (route) => {
    if (!usersHealthy) {
      return route.fulfill({ status: 500, json: { detail: 'temporary user directory failure' } })
    }
    return route.fulfill({
      json: {
        items: [
          { id: 'me-1', email: 'dev@example.com', display_name: 'Dev User', is_active: true, is_admin: true, created_at: '2026-07-01T00:00:00Z' },
          { id: 'member-1', email: 'member@example.com', display_name: 'Member', is_active: true, is_admin: false, created_at: '2026-07-02T00:00:00Z' },
          { id: 'former-1', email: 'former@example.com', display_name: 'Former', is_active: false, is_admin: false, created_at: '2026-07-03T00:00:00Z' },
        ],
        total: 3,
      },
    })
  })
  await page.route('**/api/v1/workspace-invitations', (route) => route.fulfill({
    json: {
      items: [
        { id: 'invite-pending', email: 'new@example.com', display_name: 'New Member', status: 'pending', expires_at: '2026-07-26T00:00:00Z', accepted_at: null, revoked_at: null, version: 0, created_at: '2026-07-19T00:00:00Z' },
        { id: 'invite-accepted', email: 'joined@example.com', display_name: 'Joined Member', status: 'accepted', expires_at: '2026-07-20T00:00:00Z', accepted_at: '2026-07-18T00:00:00Z', revoked_at: null, version: 1, created_at: '2026-07-12T00:00:00Z' },
      ],
      total: 2,
    },
  }))
  await page.route('**/api/v1/workspace/calendar', (route) => route.fulfill({
    json: {
      working_weekdays: [0, 1, 2, 3, 4],
      holidays: ['2026-08-17'],
      revision: 3,
  ...
  await page.route('**/api/v1/workspace/project-phase-definitions', (route) => route.fulfill({
    json: {
      items: [
        { key: 'initiation', name: '착수', color: 'sky', position: 0, retired: false, built_in: true },
        { key: 'planning', name: '계획', color: 'indigo', position: 1, retired: false, built_in: true },
        { key: 'delivery', name: '실행', color: 'emerald', position: 2, retired: false, built_in: true },
        { key: 'closure', name: '종료', color: 'amber', position: 3, retired: false, built_in: true },
        { key: 'validation', name: '검증', color: 'sky', position: 4, retired: false, built_in: false },
      ],
      revision: 5,
      updated_by_user_id: 'me-1',
      updated_by_name: 'Dev User',
  ...
  await page.route('**/api/v1/workspace/capabilities', (route) => route.fulfill({
    json: {
      wiki: { enabled: true, revision: 2 },
      ai: { enabled: true, revision: 4, deployment_enabled: false, effective_enabled: false },
      initiatives: { enabled: true, revision: 3 },
      releases: { enabled: true, revision: 2 },
      customers: { enabled: false, revision: 1 },
    },
  }))
  ...
  await expect(page.getByLabel('워크스페이스 초대 상태').getByText('1건 대기')).toBeVisible()
  await expect(page.getByLabel('근무 일정 상태').getByText('5일 근무')).toBeVisible()
...
test('보고 표면은 모바일에서 포트폴리오와 이니셔티브를 넘침 없이 보여준다', async ({ page }) => {
        ...
        items: [
          {
            ...
            cost_total: 5000000,
            hours_total: 42.5,
            schedule_baseline_id: 'baseline-mobile',
  ...
  await expect(page.getByRole('listitem').getByText('모바일 출시 기준선')).toBeVisible()
  await expect(page.getByRole('button', { name: '기준선 상세' })).toBeVisible()
  await expectNoHorizontalOverflow(page)
  await page.screenshot({
    path: '../../docs/screenshots/redevelopment/portfolio-schedule-baseline-ui/mobile.png',
    fullPage: true,
  })
