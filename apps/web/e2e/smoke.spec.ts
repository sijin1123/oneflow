...
test('로그인 참조 UI는 8개 목표 뷰포트에서 넘침과 compact 충돌 없이 렌더링된다', async ({ page }) => {
  ...
  for (const viewport of viewports) {
    ...
    if (viewport.width >= 1366) {
      ...
      expect(panel!.width / panel!.height).toBeGreaterThanOrEqual(1.35)
      expect(panel!.width / panel!.height).toBeLessThanOrEqual(1.43)
    }
    if (viewport.width >= 881 && viewport.height <= 800) {
      const collisions = await page.evaluate(() => {
        const box = (selector: string) => {
          const rect = document.querySelector(selector)?.getBoundingClientRect()
          if (!rect) throw new Error(`Missing compact login element: ${selector}`)
        ...
        const headline = box('.of-login-story-copy h1')
        const intro = box('.of-login-story-copy > p')
        const kanban = box('.of-login-kanban')
        const calendar = box('.of-login-calendar')
        const activity = box('.of-login-activity')
        return {
          headlineKanban: overlaps(headline, kanban),
          introKanban: overlaps(intro, kanban),
          introCalendar: overlaps(intro, calendar),
          calendarActivity: overlaps(calendar, activity),
        }
      })
      expect(collisions).toEqual({
        headlineKanban: false,
        introKanban: false,
        introCalendar: false,
        calendarActivity: false,
      })
    }
    await expectNoHorizontalOverflow(page)
    await page.screenshot({
      path: `../../docs/screenshots/redevelopment/login-fidelity-closure-ui/${viewport.name}.png`,
      fullPage: true,
    })
  }
})
...
test('로그인 정밀 비주얼은 선명한 보드와 흐르는 협업 경로를 유지한다', async ({ page }) => {
  ...
  await expect(page.locator('.of-login-kanban')).toHaveCSS('animation-name', 'none')
  ...
  await expect(page.locator('.of-login-collaboration-route-base')).toHaveCSS(
    'stroke-width',
    '4.4px',
  )
  const routeFlow = page.locator('.of-login-collaboration-route-flow')
  ...
  await expectNoHorizontalOverflow(page)
  await page.screenshot({
    path: '../../docs/screenshots/redevelopment/login-fidelity-closure-ui/desktop.png',
    fullPage: true,
  })
  ...
  await expectNoHorizontalOverflow(page)
  await page.screenshot({
    path: '../../docs/screenshots/redevelopment/login-fidelity-closure-ui/mobile.png',
    fullPage: true,
  })
})
