...
test('Workspace Views가 Board·Table·scope·filter·sort를 같은 결과 계약으로 연결한다', async ({ page }) => {
  ...
  await page.getByRole('menuitemradio', { name: '우선순위' }).click()
...
test('Workspace Display URL은 손상된 그룹·열·표시 값을 canonical state로 복구한다', async ({ page }) => {
  await mockApi(page)
  await page.goto('/work-items?layout=table&group_by=unknown&columns=status,status,unknown&show_empty_groups=yes&show_ids=no')
