...
async def _fetch_keys_from_connection(conn) -> set[str]:
    ...
    initiative_activity_actor_image_keys = set(
        (
            await conn.execute(
                select(InitiativeActivity.actor_profile_image_storage_key).where(
                    InitiativeActivity.actor_profile_image_storage_key.is_not(None)
                )
            )
        )
        .scalars()
    ...
    intake_decision_history_actor_image_keys = set(
        (
            await conn.execute(
                select(IntakeDecisionHistory.decided_by_profile_image_storage_key).where(
                    IntakeDecisionHistory.decided_by_profile_image_storage_key.is_not(None)
                )
            )
        )
        .scalars()
