...
Claude Code guidance for the OneFlow product workspace.

## Intent

OneFlow is the company project management system that will be built as a greenfield company-owned product codebase.

The upstream folders are references:

- `../openproject/`
- `../plane/`

The implementation target is this folder:

- `../oneflow/`

Do not modify OpenProject or Plane reference instructions unless the user explicitly asks.
...
2. Greenfield architecture or implementation
3. OneFlow customization
4. Collaboration, reporting, automation, or AI/RAG planning
5. Verification or documentation

For candidate comparison, create concise analysis artifacts. For implementation, work under `oneflow/`.

## Execution-First Development Workflow

The default workflow for Claude Code and Codex is goal and checkable completion conditions → thin plan for the first safe slice → implementation → Build/Test/Runtime verification → evidence-driven fixes and repetition.
...
- Treat `plan-validator` and `/plan-validate` as optional. Broad, non-trivial, or risky work does not automatically require them.
- When useful, prefer one quick advisory or balanced round. Non-critical findings become implementation or test tasks.
- A non-passing, max-rounds, or pending plan verdict does not by itself block implementation. Only an unresolved critical issue directly affecting the next risky action blocks that action.
...
Use OpenProject as a feature/domain reference only.

Use Plane/Linear/GitHub/Notion as UI/workflow references only.

Do not copy reference source, schema, assets, branding, or exact trade dress. SAP Business One / SAP B1 integration is excluded from scope unless the user explicitly reverses that decision.

## Validation

Use checks appropriate to the OneFlow greenfield stack. For planning-only work, validation means source inspection, architecture comparison, and documented decision criteria.

Keep `docs/PROJECT_WORKLOG.md` updated from the workspace root.
