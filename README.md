...
## 기한(due-date) 알림 운영 절차 (UI-126)

담당 작업의 기한 알림(내일 마감 `due_soon` / 첫 초과 및 선택형 반복 `overdue`)을 인앱 수신함에 넣는다.
...
- **기본 폭주 없음**: 개인 설정 기본값 `0`은 '어제가 기한'인 항목만 최초 1회 알린다. 배포 시 오래된 초과 항목을 백필하지 않는다.
- **선택형 반복**: 사용자가 3/7/14일 주기를 고르면 첫 초과일을 기준으로 정확한 간격에만 다시 알린다. 실행을 놓친 날의 알림은 보충하지 않는다.
- 대상: 담당자가 현재 프로젝트 멤버이면서 활성 사용자이고, 기한 알림 토글이 켜져 있는 경우만.
- 로그 예: `created: due_soon=3` / `created: overdue=1`. exit 0=성공, 2=락 미획득.

## 프로젝트 공유 대시보드 레이아웃 (UI-127)

프로젝트 Dashboard의 위젯 표시/순서는 아래 우선순위로 결정된다.

1. 사용자가 저장한 개인 레이아웃
2. 프로젝트 소유자가 게시한 공유 레이아웃
3. OneFlow 기본 레이아웃

- 모든 현재 프로젝트 멤버(viewer 포함)는 개인 레이아웃을 저장하거나 삭제해 공유/기본 구성으로 돌아갈 수 있다. 개인 설정은 프로젝트 데이터를 바꾸지 않으므로 보관 프로젝트에서도 허용된다.
- 공유 레이아웃 게시·갱신·삭제는 활성 프로젝트의 owner만 가능하다. 공유 쓰기는 버전 충돌을 409로 거부하며, DB는 비어 있지 않은 닫힌 위젯 어휘를 강제한다.
...
Workspace Search와 커맨드 팔레트는 업로드 파일의 이름뿐 아니라 제한된 텍스트 본문도 검색한다.

- 지원 형식은 UTF-8 `text/plain`, Markdown, CSV, TSV, JSON으로 닫혀 있다. MIME parameter는 제거해 비교하며 PDF, OCR, 이미지, binary와 외부 URL/object storage 본문은 추측해 읽지 않는다.
- 새 업로드는 저장 직후 같은 요청에서 최대 512 KiB까지만 정규화·인덱싱한다. 검색 요청은 원본 blob을 다시 열지 않고 DB의 bounded derived text만 조회한다.
- migration `0100`은 기존 LocalStorage 업로드를 `pending`으로 표시한다. Files 페이지의 `본문 검색 준비`는 한 요청에 최대 100건만 처리하며 남은 건수가 있으면 반복할 수 있다.
- 상태는 `검색 가능`, `준비 필요`, `형식 제외`, `용량 제외`, `텍스트 오류`, `파일 누락`으로 명시된다. 제외/오류 파일은 이름 검색은 가능하지만 본문 검색으로 오인하지 않는다.
- 파일 결과와 재인덱스 처리 건수는 현재 프로젝트 멤버십, 프로젝트 보관 상태, 비공개 Document 작성자 가시성, Wiki 활성 정책을 동일하게 적용한다.
- DB migration `0100` 적용이 필요하다. 신규 환경변수, dependency 또는 별도 설정 UI 변경은 없다.
